...
- The first split replaces list polling with a six-node Trello-event intake and reduces the scheduled worker to its reachable database-claim graph. The event intake accepts only moves into the two approved preview lists or deliberate removal of a sent label, re-reads current card state, and enqueues one stable event-keyed database job.
...
- The 80-node worker is explicitly an interim first split, not the final target. Video preparation/generation, QC, offer delivery, and projection still require separate stage workers below the 30-node boundary.
- Production cutover now runs event intake `o3Lckpd5ZiH1hQ4H` and database worker `S4gjf0YeZjP0pqFR`; both published graphs match their artifacts exactly and have zero strict errors. Worker version `ed957045-36e2-42c0-b5ed-fc0d4a86f19c` uses token-bound claim/finish v2 plus a downstream finish assertion. The 124-node and 74-node pollers are inactive and clearly renamed. Natural empty-queue runs use four nodes and about 5 KB instead of downloading 548 or 349 Trello cards.
- Token-bound queue v2 adds claim-replay idempotency by n8n execution, a unique token for every new lease, receipt-gated completion, stale-finish rejection, append-only transition events, and a downstream assertion node. Its rollback preserves all evidence and only switches executor permissions back to the legacy functions.
- A live scan of all 146 active workflows found no remaining legacy preview claim/finish reference and exactly one v2 consumer (`S4gjf0YeZjP0pqFR`). Commit `1c834105446d98a02e464c10e36b5fdf8cca92b4` passed safe push and `codex-predeploy ops`; production now denies legacy claim/finish execution even to `service_role`, while v2 claim/finish and event-intake enqueue remain executable.
...
49. The legacy preview claim/finish functions remained executable by `service_role` during the reversible v2 transition. A complete scan of all 146 active graphs proved no remaining consumer before production permission retirement; direct role calls now fail with PostgreSQL `42501` while v2 and enqueue remain available.
50. The initial legacy-retirement database test failed because its isolated fixture did not define the two enqueue overloads whose continued access the test was intended to protect. The fixture now models both overloads, and a fresh PostgreSQL 17 migration/rollback/reapply cycle passes.
...
- Preview legacy-RPC retirement tests pass on a fresh PostgreSQL 17 instance: migration chain, token suite, retirement, six legacy denials, preservation of v2/enqueue access, rollback restoring only service-role legacy access, and clean reapply. Production catalog checks and direct `service_role` calls confirm both legacy functions are denied; scheduled execution `3368044` then completed through v2 in 207 ms with four nodes and about 5 KB.
