...
17. First future write: one approved non-reference domestic order and then one approved EU order, serialized lease, operator witness, delivery-note print evidence before EU purchase, EasyDPD reconciliation, Shopify tracking confirmation, PDF scan test, physical print confirmation and cost check. Stop immediately on any mismatch.
