...
13. On the office print hosts, install only CUPS client tooling, configure `Brother_QL_1110NWB` with `4x6` for A6 labels and the physically separate `HP_Color_LaserJet_Pro_MFP_3302` with `A4` for delivery notes, inspect their media keywords and run `npm run arrival-labels:print-worker -- --self-test` for each environment; neither self-test may print a page.
