...
- Shopify shop `Neontrip`: Admin GraphQL provides order, customer, note, custom-attribute, shipping-line and fulfillment evidence. The admin URL is constructed from the validated `*.myshopify.com` domain and numeric order GID; untrusted note URLs are never reused.
