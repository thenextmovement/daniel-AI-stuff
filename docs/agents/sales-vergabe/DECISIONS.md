...
| Key-Kunden-Markierung ist eine deterministische Trello-Projektion aus Shopify-/Postgres-Daten. | `[lokal verifiziert; Release der Teil-Erstattungs-Korrektur offen, 2026-09-21]` | Exakte Firmendomain, mindestens zwei frühere unterschiedliche bezahlte Orders und zusammen strikt mehr als 1.200 EUR inklusive Versand. `paid` nutzt weiterhin den gespeicherten Gesamtwert. `partially_refunded` zählt nur mit positivem, aktuell von Shopify bestätigtem `netPaymentSet`; erstattete Beträge werden nicht mitgezählt. Vollständig erstattete, stornierte, unbezahlte und erst nach der Anfrage angelegte Orders bleiben ausgeschlossen. Fehlender Provider-Nachweis stoppt vor einer Titeländerung. Nur Titelprojektion, keine Zahlungsänderung. `KEY KUNDE` steht genau einmal am Anfang; übriger Titel bleibt erhalten. |
...
| Der seltene unmögliche Shopify-Link wird vor dem Merge blockiert und einmalig gemeldet. | `[im Repository verifiziert; Produktion offen]` | Die Kombination aus historischer Order, abweichender Kunden-E-Mail und abweichendem Bruttobetrag speichert einen Guard-Marker und erzeugt einmalig eine interne Mail an `info@neontrip.de`; der Sale bleibt unverknüpft, Shopify und Trello bleiben unverändert. |
...
- `[separat lokal vorbereitet; Aktivierung erst nach natürlichem 2A-Nachweis]` 2B verwendet ausschließlich vollständig gebundene bezahlte Rechnungsbelege bis höchstens fünf Minuten ab Claimstart wieder. Fehlende Rohrevision bedeutet Live-Prüfung, kein Default-Cache. Ein Treffer schreibt die Jobzeile nicht neu; geschlossene Fälle werden ohne neuen relevanten Handoff nicht regelmäßig erneut abgefragt. Der zweite Rollback erhält den kanonischen Zustand.
