...
- Die Arbeitskarten zeigen und bearbeiten jetzt direkt den Designkontext aus Trello: das Custom-Field `Usage` sowie die Kartenbeschreibung als `Description (Hintergrund/Szenario für KI-Mockups)`. Diese Werte bleiben Trello-Arbeitskontext; Postgres bleibt die Quelle der Wahrheit für Kunden-, Anfrage- und Call-Logik.
- Die Call-Zentrale ist für den kleinen 2-Personen-Betrieb gehärtet: Beim Speichern wird der zuletzt bekannte Call-Result-Stand geprüft; wenn jemand schneller war, kommt eine Aktualisieren-Meldung statt stilles Überschreiben. Bearbeitete Fälle verschwinden nicht kommentarlos, sondern stehen in der Call-Zentrale unter `Heute bearbeitet`.
- Der angezeigte Operatorname stammt aktuell aus dem internen Login-/Browserfeld und ist keine extern geprüfte Identität. Für Tageslisten-Calls ist die Call-Zentrale verbindlich; Customer Records öffnet für Anrufe nur noch die Call-Zentrale mit der passenden Request-ID. Der alte Audit-Log-Callpfad bleibt als Legacy-Fallback im Backend, wird aber nicht mehr als primäre UI-Aktion angeboten.
...
- Die Verknuepfung erfolgt automatisch ueber `Offer.trelloCardId`; wenn diese fehlt, kann man Angebote manuell nach Angebotsnummer, Angebotslink, E-Mail, Firma, Name oder Trello-Link suchen. Trello bleibt damit nur Zuordnung/Projektion, nicht Angebots-Datenbank.
- Manuell geoeffnete Angebote werden nicht automatisch mit dem Customer-Record verknuepft.
...
- Nach dem Speichern kann ein aktualisiertes Angebot separat per E-Mail erneut gesendet werden. Speichern loest keinen automatischen Versand aus. Der Versand ist blockiert, wenn die Kunden-E-Mail im Angebot nicht zur E-Mail des geoeffneten Datensatzes passt.
