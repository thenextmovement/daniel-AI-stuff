...
begin
  ...
  -- Ops may edit only the fields shown in its current UI. Preserve every other
  -- customer-provided address detail so accepting a request never drops names,
  -- delivery instructions, or future address fields.
  v_reviewed := coalesce(v_original,'{}'::jsonb)
    || (coalesce(v_reviewed,'{}'::jsonb) - 'billingAddress' - 'deliveryAddress')
    || case when jsonb_typeof(v_reviewed->'billingAddress')='object' then
      jsonb_build_object(
        'billingAddress',
        coalesce(v_original->'billingAddress','{}'::jsonb) || (v_reviewed->'billingAddress')
      ) else '{}'::jsonb end
    || case when jsonb_typeof(v_reviewed->'deliveryAddress')='object' then
      jsonb_build_object(
        'deliveryAddress',
        coalesce(v_original->'deliveryAddress','{}'::jsonb) || (v_reviewed->'deliveryAddress')
      ) else '{}'::jsonb end;
