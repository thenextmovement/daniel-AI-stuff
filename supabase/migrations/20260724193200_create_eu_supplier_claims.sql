...
declare v_id uuid; begin
 ...
 select id into v_id from public.eu_supplier_deliveries where status in ('queued','retry_wait') and (next_attempt_at is null or next_attempt_at<=now()) order by created_at for update skip locked limit 1;
 if v_id is null then return; end if;
 return query update public.eu_supplier_deliveries set status='sending',attempt_count=attempt_count+1,workflow_execution_id=left(p_worker,200),updated_at=now() where id=v_id and status in ('queued','retry_wait') returning *;
end $$;
...
declare v_id uuid; begin
 if coalesce(auth.role(),'')<>'service_role' then raise exception 'service_role_required'; end if;
 select id into v_id from public.eu_supplier_deliveries where status='failed' and alert_status in ('pending','failed') order by failed_at nulls last,created_at for update skip locked limit 1;
 if v_id is null then return; end if;
 return query update public.eu_supplier_deliveries set alert_status='sending',workflow_execution_id=left(p_worker,200),updated_at=now() where id=v_id and status='failed' and alert_status in ('pending','failed') returning *;
end $$;
