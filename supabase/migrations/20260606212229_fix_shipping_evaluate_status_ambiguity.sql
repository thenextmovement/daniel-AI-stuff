...
begin
  ...
  if not public.shipping_is_recent_shipment(v_shipment.shipped_at, coalesce(v_event.event_time, v_shipment.last_event_at), v_shipment.created_at, v_shipment.updated_at, p_now) then
    ...
    set risk_level = case when public.shipping_shipments.status in ('delivered', 'closed') then 'closed' else 'normal' end,
        updated_at = p_now
    where public.shipping_shipments.id = p_shipment_id;
  ...
  if v_shipment.status in ('delivered', 'closed') then
    ...
    set status = 'resolved',
        resolved_at = coalesce(public.shipping_incidents.resolved_at, p_now),
        updated_at = p_now
    where public.shipping_incidents.shipment_id = p_shipment_id
      and public.shipping_incidents.status in ('open', 'acknowledged');
