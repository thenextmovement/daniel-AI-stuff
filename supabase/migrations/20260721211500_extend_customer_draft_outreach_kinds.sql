-- Extend the fail-closed customer draft ledger to post-delivery and repeat-business
-- outreach. All supported kinds remain draft-only and require human sending.

alter table public.customer_communication_draft_jobs
  drop constraint customer_communication_draft_jobs_communication_kind_check;

alter table public.customer_communication_draft_jobs
  add constraint customer_communication_draft_jobs_communication_kind_check
  check (communication_kind in (
    'design_reminder',
    'winback',
...
begin
  if safe_kind not in ('design_reminder', 'winback', 'activecampaign_autoreply', 'post_delivery', 'repeat_business') then
...
comment on function public.claim_customer_communication_draft(text, text, text, text, integer) is
  'Claims one customer draft job atomically. Supports all approved customer outreach draft kinds; never authorizes automatic sending.';
