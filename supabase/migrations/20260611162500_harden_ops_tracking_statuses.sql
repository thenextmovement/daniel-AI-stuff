...
begin
  if v_code = '' and btrim(v_text) = '' then
  ...
  if v_code in ('OC') or v_text ~ 'shipment information sent|shipment information received|pre[-\s]*advice|inforeceived|info received|label created|label generated|sendungsdaten|daten.*uebermittelt|daten.*übermittelt' then
...
begin
  ...
  if v_shipment.status = 'carrier_not_found' and v_hours_since_tracking >= 72 then
    v_candidates := v_candidates || jsonb_build_array(jsonb_build_object(
      'type', 'carrier_not_found',
      'severity', 'high',
      'title', 'Carrier kennt die Sendung nicht',
      'description', 'Die Trackingnummer ist seit mindestens 72 Stunden bekannt, aber der Carrier findet keine Sendung.'
    ));
  end if;
  ...
  if v_shipment.status in ('in_transit', 'tendered') and v_hours_since_movement >= 72 then
    v_candidates := v_candidates || jsonb_build_array(jsonb_build_object(
      ...
      'description', 'Seit mindestens 72 Stunden nach echter Übergabe gibt es kein neues Tracking-Event.'
    ));
  end if;
  ...
  loop
    ...
    values (
      ...
      p_now,
      p_now,
      'inbound_shipping_rules_v2_20260611',
      v_latest.id,
    ...
    on conflict on constraint inbound_incidents_incident_key_key do update
      set severity = excluded.severity,
          status = case when target.status in ('resolved', 'ignored') then target.status else excluded.status end,
...
begin
  with candidates as (
    ...
    where i.status in ('open', 'acknowledged')
      and i.incident_type in ('clearance_action_required', 'out_for_delivery', 'stale_no_movement', 'carrier_exception', 'carrier_not_found', 'tracking_error')
      and s.status not in ('tracking_created', 'label_created')
  ...
  select
    'inbound:incident:' || c.incident_id::text || ':initial',
    c.shipment_id,
    c.incident_id,
    'info@neontrip.de',
    case
