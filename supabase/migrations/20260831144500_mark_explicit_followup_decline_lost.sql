...
begin
  if md5(pg_get_functiondef(to_regprocedure(
    ...
    raise exception using errcode = '55000',
      message = 'followup_reply_decision_source_drifted';
  end if;
end;
$precondition$;
...
declare
  ...
  deal_status_before text;
  deal_status_after text;
  deal_status_reason text;
  deal_marked_lost boolean := false;
begin
  ...
  if attempt.id is null
    ...
    if source_row.email_context_snapshot->>'reply_workflow_execution_id'
      ...
      return jsonb_build_object(
        ...
        'deal_marked_lost', coalesce(
          (source_row.email_context_snapshot->>'deal_marked_lost')::boolean,
          false
        ),
        'deal_status_after', source_row.email_context_snapshot->>'deal_status_after'
      );
    end if;
  ...
  if safe_decision = 'DECLINED' then
    select
      mr.id,
      lower(coalesce(nullif(btrim(mr.deal_status), ''), 'open'))
    into resolved_request_id, deal_status_before
    from public.master_requests mr
    where mr.id::text = source_row.request_id
       or mr.request_id = source_row.request_id
    order by (mr.id::text = source_row.request_id) desc,
             mr.created_at desc nulls last
    limit 1
    for update;

    if not found then
      deal_status_reason := 'request_not_resolved';
    elsif deal_status_before = 'open' then
      update public.master_requests
        set deal_status = 'lost',
            updated_at = now()
      where id = resolved_request_id
        and lower(coalesce(nullif(btrim(deal_status), ''), 'open')) = 'open'
      returning lower(deal_status) into deal_status_after;
      deal_marked_lost := found;
      deal_status_reason := case
        when deal_marked_lost then 'explicit_decline'
        else 'concurrent_status_change_protected'
      end;
    elsif deal_status_before = 'lost' then
      deal_status_after := 'lost';
      deal_status_reason := 'already_lost';
    else
      deal_status_after := deal_status_before;
      deal_status_reason := 'existing_terminal_state_protected';
    end if;

    if deal_status_after is null and resolved_request_id is not null then
      select lower(coalesce(nullif(btrim(mr.deal_status), ''), 'open'))
      into deal_status_after
      from public.master_requests mr
      where mr.id = resolved_request_id;
    end if;

    reply_snapshot := reply_snapshot || jsonb_build_object(
      'deal_marked_lost', deal_marked_lost,
      'deal_status_before', deal_status_before,
      'deal_status_after', deal_status_after,
      'deal_status_reason', deal_status_reason
    );
...
comment on function public.apply_followup_reply_decision(uuid,uuid,text,text,text,text,numeric,text,text,text,text) is
  'Claim-bound reply action: deterministic explicit decline cancels follow-ups and marks an open canonical request lost; timing replies snooze; ambiguous replies require human review.';
...
begin
  if md5(definition) <> 'cf7eabf48a68c007b0e6626d838e9ec9'
    ...
    raise exception using errcode = '55000',
      message = 'followup_decline_lost_postcondition_failed';
  end if;
end;
$postcondition$;
