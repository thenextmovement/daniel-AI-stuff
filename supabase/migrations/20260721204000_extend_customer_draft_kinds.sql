-- Extend the fail-closed customer draft ledger to ActiveCampaign auto-reply
-- candidates. This remains draft-only and requires human sending.
...
begin
  if safe_kind not in ('design_reminder', 'winback', 'activecampaign_autoreply') then
...
comment on function public.claim_customer_communication_draft(text, text, text, text, integer) is
  'Claims one customer draft job atomically. Supports design reminder, win-back, and ActiveCampaign auto-reply drafts; never authorizes automatic sending.';
