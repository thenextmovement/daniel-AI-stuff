create or replace function public.shipping_is_recent_shipment(
...
stable
as $$
  select coalesce(p_shipped_at, p_last_event_at, p_created_at, p_updated_at) is null
      or coalesce(p_shipped_at, p_last_event_at, p_created_at, p_updated_at) >= p_now - interval '60 days';
$$;
...
begin
  with pickup_candidates as (
    ...
    where i.incident_type = 'pickup_available'
      ...
      and public.shipping_is_recent_shipment(s.shipped_at, coalesce(e.event_time, s.last_event_at), s.created_at, s.updated_at, p_now)
...
revoke all on function public.shipping_is_recent_shipment(timestamptz, timestamptz, timestamptz, timestamptz, timestamptz) from public, anon, authenticated;
...
grant execute on function public.shipping_is_recent_shipment(timestamptz, timestamptz, timestamptz, timestamptz, timestamptz) to service_role;
