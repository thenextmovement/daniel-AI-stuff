...
as $$
  ...
  select case
    ...
    when text ~ '\bsuccess\b|fulfilled' then 'label_created'
...
set normalized_status = 'label_created',
    mapping_version = 'carrier_status_mapping_v3_20260623_shopify_success'
where carrier_status_code = 'SUCCESS'
  and lower(coalesce(carrier_status_text, '')) = 'fulfilled'
  and normalized_status = 'carrier_not_found';
...
set status = 'label_created',
    risk_level = public.shipping_risk_level('label_created'),
    updated_at = now()
from latest_success e
where s.id = e.shipment_id
  and s.status = 'carrier_not_found';
...
with latest_success as (
  ...
  where carrier_status_code = 'SUCCESS'
    and lower(coalesce(carrier_status_text, '')) = 'fulfilled'
    and mapping_version = 'carrier_status_mapping_v3_20260623_shopify_success'
  order by shipment_id, event_time desc, created_at desc
)
update public.shipping_incidents i
set status = 'resolved',
    resolved_at = coalesce(i.resolved_at, now()),
    updated_at = now(),
    metadata = coalesce(i.metadata, '{}'::jsonb) || jsonb_build_object('resolved_by', 'carrier_status_mapping_v3_20260623_shopify_success')
from latest_success e
where i.shipment_id = e.shipment_id
  and i.incident_type = 'carrier_not_found'
