...
begin
  ...
  if coalesce(array_length(p_job_types,1),0)=0 or not (p_job_types <@ array['CREATE_PROFORMA','CREATE_INVOICE','CREATE_CREDIT','CREATE_CANCELLATION','VOID_PROFORMA','PROJECT_PAYMENT_SHOPIFY','PROJECT_PAYMENT_EASYBILL','SEND_CUSTOMER_DOCUMENT','VERIFY_VAT','RECONCILE']::text[]) then raise exception 'BILLING_JOB_TYPES_INVALID'; end if;
  ...
  if p_success then
    ...
    if v_job.job_type in ('CREATE_PROFORMA','CREATE_INVOICE','CREATE_CREDIT','CREATE_CANCELLATION') then
      ...
      insert into public.billing_documents (billing_case_id,document_type,revision,document_number,status,easybill_document_id,payload_hash,amount_cents,currency,finalized_at,sent_at)
        values (v_case.id,v_type,v_revision,v_number,case when coalesce((p_result->>'sent')::boolean,false) then 'SENT' else 'FINALIZED' end,v_easybill_id,coalesce(nullif(p_result->>'payloadHash',''),encode(digest(v_job.payload::text,'sha256'),'hex')),case when v_type='CREDIT' then (v_job.payload->>'amountCents')::bigint when v_type='CANCELLATION' then coalesce((v_job.payload->>'invoiceAmountCents')::bigint,v_case.total_gross_cents) else v_case.total_gross_cents end,v_case.currency,now(),case when coalesce((p_result->>'sent')::boolean,false) then now() else null end)
