-- Add a bounded attachment signal to the existing NEONTRIP relationship lookup.
--
-- The delivery workflow must never infer a missing file from Outlook mail state.
-- Only persisted form rows with an explicit empty file_urls array are classified
-- as missing. Unknown or non-form sources keep the normal acknowledgement.
