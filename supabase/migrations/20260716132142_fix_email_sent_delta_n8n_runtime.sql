...
begin
  ...
  if current_state.lease_owner is not null
    and current_state.lease_until is not null
    and current_state.lease_until <= now() then
    update public.email_agent_mail_sync_runs
    set status = 'error',
        error_code = 'lease_expired',
        error_message = 'Workflow ended before recording its Microsoft Graph result.',
        completed_at = now()
    where mailbox_key = mailbox_value
      and execution_id = current_state.lease_owner
      and status = 'running';
  end if;
...
comment on function public.begin_email_agent_sent_sync_v1(text, text) is
  'Claims an idempotent Sent Items delta lease and closes an expired prior run before the next execution.';
