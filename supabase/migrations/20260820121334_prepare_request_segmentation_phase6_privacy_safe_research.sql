-- Prepare the additive NEONTRIP Phase-6 privacy-safe research pilot.
--
-- This migration is intentionally DB-first and non-operational:
--   * Phase-2 policy/gate remain the sole active contract.
--   * Phase-6 policy/gate and all eight rules are inserted inactive/inert.
...
--   * The existing 18-argument Record RPC and the cache writer are untouched.
...
begin
  ...
  if v_active_policy_count <> 1
    ...
    raise exception using
      errcode = '55000',
      message = 'phase6_base_requires_exact_active_phase2_shadow_contract',
      detail = format(
  ...
  if v_candidate_object_count <> 0 then
    raise exception 'phase6_candidate_contract_already_exists';
  end if;
  ...
  if v_candidate_job_count <> 0
    ...
    raise exception using
      errcode = '55000',
      message = 'phase6_base_requires_pristine_candidate_runtime',
      detail = format(
...
select
  'nt_quality_gate_v4_20260820_cx8',
  q.taxonomy_version,
  'segment_classifier_v5_20260820_cx8',
  'segment_prompt_v5_20260820_cx8',
  false,
  ...
  concat(
    ...
    'segment_research_v1_20260820_cx8; research model ',
    'gpt-4o-mini-2024-07-18; classifier model gpt-5.5-2026-04-23 ',
    'with medium reasoning; validator n8n_cx8_validator_v2. Thresholds ',
...
insert into public.segment_policy_versions (
  ...
  'nt_policy_v4_20260820_cx8_shadow',
  false,
  'shadow',
  'codex-phase6',
  concat(
    'Evaluation-only Phase-6 candidate. It is not a normal ingress policy; ',
...
create function public.neontrip_phase6_evaluation_contract_is_exact()
returns boolean
language sql
stable
...
as $function$
  select
    ...
    and (
      select count(*) = 8
      from public.segment_policy_rules r
      where r.policy_version = 'nt_policy_v4_20260820_cx8_shadow'
        and r.taxonomy_version = 'nt_taxonomy_v2_20260819_cx8'
    )
    and not exists (
      select 1
...
comment on function public.neontrip_phase6_evaluation_contract_is_exact() is
  'Single fail-closed runtime guard shared by Phase-6 Claim, Payload and Record: exact sole active Phase-2 contract plus exact inactive Phase-6 contract and eight fully inert candidate rules.';

revoke all on function public.neontrip_phase6_evaluation_contract_is_exact()
  from public, anon, authenticated, service_role;

create function public.neontrip_phase6_redact_segmentation_text(
  p_value text,
...
comment on function public.neontrip_phase6_redact_segmentation_text(text, integer, text[]) is
  'Internal Phase-6 minimizer: removes known contact values and generic email, URL, phone, UUID and opaque-ID patterns before a hard text bound. It is not a general anonymizer and is exposed only through the exact evaluation payload RPC.';

revoke all on function public.neontrip_phase6_redact_segmentation_text(text, integer, text[])
  from public, anon, authenticated, service_role;
...
create function public.neontrip_phase6_evaluation_research_context(p_job_id uuid)
returns jsonb
language sql
volatile
security definer
...
as $function$
  with exact_job as (
    ...
    from public.request_segmentation_jobs j
    where j.id = p_job_id
      and j.source = 'gold_re_evaluation_phase6'
      and j.status = 'processing'
      and j.lock_owner = 'n8n-request-segmenter-v5'
      ...
      and j.metadata->>'policy_version' = 'nt_policy_v4_20260820_cx8_shadow'
      and j.metadata->>'quality_gate_version' = 'nt_quality_gate_v4_20260820_cx8'
      and j.metadata->>'research_contract' = 'segment_research_v1_20260820_cx8'
      and j.metadata->>'validator_version' = 'n8n_cx8_validator_v2'
      ...
      and public.neontrip_phase6_evaluation_contract_is_exact()
    ...
    select
      p.*,
      (
        coalesce((p.domain_facts->>'is_valid_dns_host')::boolean, false)
        or p.safe_company is not null
      ) as external_research_required,
      case
        when p.domain_lookup_allowed then concat(
          'site:', p.domain_facts->>'email_domain',
          ' Unternehmen Leistungen Kundenprojekte Standorte Impressum'
        )
        when p.company_lookup_allowed then concat(
          p.safe_company,
          ' offizielle Website Unternehmen Leistungen Kundenprojekte Standorte'
        )
        else null
  ...
  select coalesce(
    (
      select jsonb_build_object(
        'context_valid', true,
        -- A screened value is released only when it is also authorized for the
        -- deterministic company lookup. Ambiguous/non-business values stay
        -- NULL, so the model never receives a person-like fallback string.
        'company', case when ep.company_lookup_allowed then ep.safe_company else null end,
        'company_lookup_allowed', ep.company_lookup_allowed,
...
comment on function public.neontrip_phase6_evaluation_research_context(uuid) is
  'Internal, ID-free Phase-6 lookup planner shared by Payload and Record. Domain lookup is preferred; company fallback requires a screened explicit legal/business marker. The exact deterministic query is never caller supplied.';

revoke all on function public.neontrip_phase6_evaluation_research_context(uuid)
  from public, anon, authenticated, service_role;

create function public.neontrip_claim_request_segmentation_phase6_evaluation(
  p_limit integer default 1,
  p_lock_owner text default 'n8n-request-segmenter-v5',
...
as $function$
declare
  v_lock_owner constant text := 'n8n-request-segmenter-v5';
begin
  if p_limit is distinct from 1 then
    raise exception 'phase6_evaluation_claim_limit_must_equal_1';
  end if;

  if coalesce(nullif(btrim(p_lock_owner), ''), v_lock_owner) <> v_lock_owner then
    raise exception 'phase6_evaluation_claim_lock_owner_mismatch';
  end if;

  if coalesce(p_stale_minutes, 0) < 1 or p_stale_minutes > 60 then
    raise exception 'phase6_evaluation_claim_stale_minutes_must_be_between_1_and_60';
  end if;

  if not public.neontrip_phase6_evaluation_contract_is_exact() then
    raise exception 'phase6_evaluation_runtime_contract_drift';
  end if;
  ...
  with picked as (
    select j.id
    from public.request_segmentation_jobs j
    where j.source = 'gold_re_evaluation_phase6'
      and j.taxonomy_version = 'nt_taxonomy_v2_20260819_cx8'
      and j.classifier_version = 'segment_classifier_v5_20260820_cx8'
      and j.prompt_version = 'segment_prompt_v5_20260820_cx8'
      and lower(coalesce(j.metadata->>'evaluation_only', 'false')) = 'true'
...
comment on function public.neontrip_claim_request_segmentation_phase6_evaluation(integer, text, integer) is
  'Claims exactly one item per call from the four-job Phase-6 immutable-Gold evaluation lane while the candidate remains inactive and Phase 2 remains the sole active contract. It cannot claim normal ingress.';

revoke all on function public.neontrip_claim_request_segmentation_phase6_evaluation(integer, text, integer)
  from public, anon, authenticated, service_role;
grant execute on function public.neontrip_claim_request_segmentation_phase6_evaluation(integer, text, integer)
  to service_role;

create function public.neontrip_get_request_segmentation_phase6_evaluation_payload(
  p_job_id uuid
)
returns jsonb
language sql
...
as $function$
  ...
  runtime_contract as (
    select public.neontrip_phase6_evaluation_contract_is_exact() as valid
  ),
  req as (
    select mr.*
  ...
  research_context as (
    select public.neontrip_phase6_evaluation_research_context(j.id) as value
    from exact_job j
  ),
  minimized_input as (
    select jsonb_build_object(
      'title', public.neontrip_phase6_redact_segmentation_text(r.title, 240, rl.sensitive_values),
      'description', public.neontrip_phase6_redact_segmentation_text(r.description, 1600, rl.sensitive_values),
      -- The four Phase-6 Gold inputs have no raw first-party customer-type
      ...
      'application', public.neontrip_phase6_redact_segmentation_text(r.application, 160, rl.sensitive_values),
      'country', public.neontrip_phase6_redact_segmentation_text(coalesce(r.country, c.country), 80, rl.sensitive_values),
  ...
  select case
    when not exists (select 1 from job) then
      jsonb_build_object('payload_error', jsonb_build_object('code', 'phase6_evaluation_job_not_found'))
    when not exists (select 1 from exact_job) then
      jsonb_build_object('payload_error', jsonb_build_object('code', 'phase6_evaluation_job_contract_mismatch'))
    when not coalesce((select valid from runtime_contract), false)
      ...
      jsonb_build_object('payload_error', jsonb_build_object('code', 'phase6_evaluation_runtime_contract_drift'))
    when not exists (select 1 from req) then
      jsonb_build_object('payload_error', jsonb_build_object('code', 'phase6_request_not_found'))
    when not exists (select 1 from customer) then
      jsonb_build_object('payload_error', jsonb_build_object('code', 'phase6_customer_not_found'))
    else jsonb_build_object(
      'contract', jsonb_build_object(
        'taxonomy_version', 'nt_taxonomy_v2_20260819_cx8',
        'classifier_version', 'segment_classifier_v5_20260820_cx8',
        'prompt_version', 'segment_prompt_v5_20260820_cx8',
        'policy_version', 'nt_policy_v4_20260820_cx8_shadow',
        'quality_gate_version', 'nt_quality_gate_v4_20260820_cx8',
        'research_contract', 'segment_research_v1_20260820_cx8',
        'source', 'gold_re_evaluation_phase6',
        'evaluation_only', true,
        'master_projection_authorized', false,
        'validator_version', 'n8n_cx8_validator_v2',
...
comment on function public.neontrip_get_request_segmentation_phase6_evaluation_payload(uuid) is
  'Returns an ID-free, no-cache, no-history, no-Gold Phase-6 model payload. The caller must retain job_id/request_id/input_hash from Normalize Claim through per-item n8n lineage; exact claim-to-payload pairing is a mandatory release proof.';

revoke all on function public.neontrip_get_request_segmentation_phase6_evaluation_payload(uuid)
  from public, anon, authenticated, service_role;
grant execute on function public.neontrip_get_request_segmentation_phase6_evaluation_payload(uuid)
  to service_role;

-- A 19-argument overload keeps the existing 18-argument production Record
-- function byte-identical. PostgREST resolves this pilot-only overload by the
-- additional required named argument p_research_contract.
...
declare
  ...
  v_phase6_job_identity_valid boolean := false;
  ...
  v_first_party_customer_type_verified constant boolean := false;
  v_private_declaration_evidence_valid boolean := false;
  v_job_status text;
begin
  if p_job_id is null then
    raise exception 'phase6_evaluation_record_requires_job_id';
  end if;
  ...
  if pg_column_size(v_classifier_json) > 262144
    ...
    raise exception 'phase6_evaluation_record_input_bounds_exceeded';
  end if;
  ...
  if not found then
    raise exception 'phase6_evaluation_job_not_found';
  end if;

  v_phase6_job_identity_valid :=
    v_job.source = 'gold_re_evaluation_phase6'
    and v_job.taxonomy_version = 'nt_taxonomy_v2_20260819_cx8'
    and v_job.classifier_version = 'segment_classifier_v5_20260820_cx8'
    and v_job.prompt_version = 'segment_prompt_v5_20260820_cx8'
    ...
    and v_job.metadata->>'policy_version' = 'nt_policy_v4_20260820_cx8_shadow'
    and v_job.metadata->>'quality_gate_version' = 'nt_quality_gate_v4_20260820_cx8'
    and v_job.metadata->>'research_contract' = 'segment_research_v1_20260820_cx8'
    and v_job.metadata->>'validator_version' = 'n8n_cx8_validator_v2'
  ...
  if not found then
    raise exception 'phase6_evaluation_job_not_found_after_lock';
  end if;
  ...
  if not v_phase6_job_identity_valid
    ...
    raise exception using
      errcode = '22023',
      message = 'phase6_evaluation_record_job_identity_changed_before_lock';
  end if;
  ...
  if not v_input_hash_current then
    raise exception using
      errcode = '55000',
      message = 'phase6_evaluation_record_current_input_hash_mismatch';
  end if;
  ...
  if v_job.status in ('completed', 'needs_review') then
    if v_job.lock_owner is not null
       or not public.neontrip_phase6_evaluation_contract_is_exact()
       or p_prompt_version <> 'segment_prompt_v5_20260820_cx8'
       or p_classifier_version <> 'segment_classifier_v5_20260820_cx8'
       or p_accepted_by <> 'n8n-request-segmenter-v5'
       or p_research_contract <> 'segment_research_v1_20260820_cx8'
       ...
       or v_classifier_json->>'classifier_version' <> 'segment_classifier_v5_20260820_cx8'
       or v_classifier_json->>'prompt_version' <> 'segment_prompt_v5_20260820_cx8'
       or v_classifier_json->>'research_contract' <> 'segment_research_v1_20260820_cx8'
       or v_job.last_classification_id is null then
      raise exception 'phase6_evaluation_record_terminal_replay_mismatch';
    end if;
    ...
    where c.id = v_job.last_classification_id
      ...
      and c.classifier_version = 'segment_classifier_v5_20260820_cx8'
      and c.prompt_version = 'segment_prompt_v5_20260820_cx8'
      and c.policy_version = 'nt_policy_v4_20260820_cx8_shadow'
    ...
    if not found then
      raise exception 'phase6_evaluation_record_terminal_classification_mismatch';
    end if;
  ...
  if v_job.status <> 'processing'
     or v_job.lock_owner <> 'n8n-request-segmenter-v5' then
    raise exception 'phase6_evaluation_record_job_not_processing';
  end if;
  ...
  v_research_context := public.neontrip_phase6_evaluation_research_context(p_job_id);
  ...
  v_hard_contract_valid :=
    ...
    and v_phase6_job_identity_valid
    and public.neontrip_phase6_evaluation_contract_is_exact()
    and v_input_hash_current
    and coalesce((v_research_context->>'context_valid')::boolean, false)
    and v_job.status = 'processing'
    and v_job.lock_owner = 'n8n-request-segmenter-v5'
    and v_job.attempts > 0
    and v_job.attempts <= v_job.max_attempts
    and p_prompt_version = 'segment_prompt_v5_20260820_cx8'
    and p_classifier_version = 'segment_classifier_v5_20260820_cx8'
    and p_accepted_by = 'n8n-request-segmenter-v5'
    and p_research_contract = 'segment_research_v1_20260820_cx8'
    ...
    and v_classifier_json->>'classifier_version' = 'segment_classifier_v5_20260820_cx8'
    and v_classifier_json->>'prompt_version' = 'segment_prompt_v5_20260820_cx8'
    and v_classifier_json->>'research_contract' = 'segment_research_v1_20260820_cx8'
    and v_classifier_json->>'validator_version' = 'n8n_cx8_validator_v2'
    ...
    and v_provenance->>'research_contract' = 'segment_research_v1_20260820_cx8'
    and v_provenance->>'validator_version' = 'n8n_cx8_validator_v2'
  ...
  if not v_hard_contract_valid then
    ...
    set
      status = 'failed',
      completed_at = null,
      last_error_code = 'phase6_evaluation_record_contract_mismatch',
      last_error_message = 'Phase-6 evaluation Record rejected a lane, worker, model, research, validator, Gold, or runtime-contract mismatch.',
      updated_at = now(),
      lock_owner = null,
      locked_at = null
    where id = p_job_id
      ...
      and source = 'gold_re_evaluation_phase6'
      and status = 'processing'
      and lock_owner = 'n8n-request-segmenter-v5'
      and taxonomy_version = 'nt_taxonomy_v2_20260819_cx8'
      and classifier_version = 'segment_classifier_v5_20260820_cx8'
      and prompt_version = 'segment_prompt_v5_20260820_cx8'
      and attempts > 0
      ...
      and metadata->>'policy_version' = 'nt_policy_v4_20260820_cx8_shadow'
      and metadata->>'quality_gate_version' = 'nt_quality_gate_v4_20260820_cx8'
      and metadata->>'research_contract' = 'segment_research_v1_20260820_cx8'
      and metadata->>'validator_version' = 'n8n_cx8_validator_v2'
    ...
    return jsonb_build_object(
      ...
      'error_code', 'phase6_evaluation_record_contract_mismatch',
      'evaluation_only', true,
      'master_projection_authorized', false,
      'research_cache_written', false,
      'projection', jsonb_build_object(
  ...
  v_evidence_semantics_shape_valid := v_evidence_json_shape_valid and not exists (
    ...
    where not (evidence.value ?& array['type', 'url', 'used_for', 'evidence_code'])
       ...
       or evidence.value->>'type' not in ('web_search', 'customer_declared')
       ...
       or (evidence.value->>'type' = 'customer_declared' and jsonb_typeof(evidence.value->'url') <> 'null')
  );
  ...
  v_research_binding_valid :=
    ...
    when v_research_performed then
      ...
      and v_verified_source_count = 0
      and v_verified_source_shape_valid
      and cardinality(v_positive_codes) = 0
  end;

  -- This exact pilot has no trustworthy raw first-party customer-type proof.
  -- The stored synthetic B2B defaults and the conflictual NT-8 Gold label are
  -- diagnostic inputs only; neither may become classifier evidence.
  v_private_declaration_evidence_valid := v_first_party_customer_type_verified
    and p_segment = 'NT-8'
    ...
    and exists (
      ...
      where evidence.value->>'type' = 'customer_declared'
  ...
  v_positive_evidence_bound := case
    when p_segment = 'NT-8' then v_private_declaration_evidence_valid
    else
  ...
  v_scale_evidence_valid := case
    when p_segment not in ('NT-5', 'NT-6') then true
    else exists (
      select 1
      from jsonb_array_elements(v_verified_sources) source(value)
      join jsonb_array_elements(
  ...
  if p_status = 'accepted' and p_segment <> 'NT-8' and not v_research_performed then
    v_effective_status := 'needs_review';
  ...
  set
    ...
    last_error_code = case when v_job_status = 'failed' then 'phase6_classifier_error' else null end,
...
comment on function public.neontrip_record_request_segment_classification(
  ...
  'Phase-6-only 19-argument Record overload. Exact inactive eval lane, immutable Gold, worker, model, research, validator and provenance markers are mandatory. It can write only the candidate classification and terminal job state; master/cache/customer actions have no code path.';
...
create view public.request_segmentation_v4_gold_evaluation
with (security_invoker = true)
as
with target_contract as (
  select q.*
  from public.segment_quality_gate_versions q
  where q.version = 'nt_quality_gate_v4_20260820_cx8'
    and not q.active
    and q.taxonomy_version = 'nt_taxonomy_v2_20260819_cx8'
    and q.classifier_version = 'segment_classifier_v5_20260820_cx8'
    and q.prompt_version = 'segment_prompt_v5_20260820_cx8'
    and q.min_unique_gold_total = 300
    and q.min_gold_per_segment = 25
    and q.min_precision_per_predicted_class = 0.90
    and q.min_recall_per_actual_class = 0.85
  ...
  select
    ...
    (
      coalesce((rp.domain_facts->>'is_valid_dns_host')::boolean, false)
      or rp.safe_company is not null
    ) as external_research_required,
    case
      ...
      when rp.company_lookup_allowed then concat(
        rp.safe_company,
        ' offizielle Website Unternehmen Leistungen Kundenprojekte Standorte'
      )
      else null
  ...
  select
    ...
    coalesce(
      c.classifier_json->>'research_contract' = 'segment_research_v1_20260820_cx8'
      and c.classifier_json->>'validator_version' = 'n8n_cx8_validator_v2'
      and c.classifier_json->>'research_model' = 'gpt-4o-mini-2024-07-18'
      and c.classifier_json->'evidence_provenance'->>'research_contract' = 'segment_research_v1_20260820_cx8'
      and c.classifier_json->'evidence_provenance'->>'validator_version' = 'n8n_cx8_validator_v2'
...
comment on view public.request_segmentation_v4_gold_evaluation is
  'Exact Phase-6 classifier-v5 evaluation. Gold/prediction join on request, current immutable input hash, taxonomy, classifier, prompt and candidate policy; model/research/validator drift is explicit and fail closed.';

create view public.request_segmentation_v4_confusion_matrix
with (security_invoker = true)
as
select
  actual_segment,
...
from public.request_segmentation_v4_gold_evaluation
group by
  actual_segment,
  case
    when classifier_status = 'accepted'
...
comment on view public.request_segmentation_v4_confusion_matrix is
  'Phase-6 confusion matrix. Contract-invalid accepted rows are abstentions, never valid predictions.';

create view public.request_segmentation_v4_segment_quality
with (security_invoker = true)
as
with target_contract as (
  select q.*
  ...
  from public.request_segmentation_v4_gold_evaluation e
...
comment on view public.request_segmentation_v4_segment_quality is
  'Per-class Phase-6 precision/recall. Abstentions and contract-invalid rows lower recall/coverage.';

create view public.request_segmentation_v4_quality_summary
with (security_invoker = true)
as
with target_contract as (
  select q.*
  ...
  select * from public.request_segmentation_v4_gold_evaluation
)
select
  tc.taxonomy_version,
  tc.classifier_version,
  tc.prompt_version,
  tc.version as quality_gate_version,
  'segment_research_v1_20260820_cx8'::text as research_contract,
...
comment on view public.request_segmentation_v4_quality_summary is
  'Phase-6 totals with exact model/research contract violations separated from accepted metrics.';

create view public.request_segmentation_v4_mapping_integrity
with (security_invoker = true)
as
with target_contract as (
  select q.*
  ...
  left join public.segment_policy_rules r
    on r.policy_version = 'nt_policy_v4_20260820_cx8_shadow'
...
join public.request_segmentation_v4_quality_summary qs
  on qs.quality_gate_version = tc.version;

comment on view public.request_segmentation_v4_mapping_integrity is
  'Exact eight-definition/eight-inert-rule mapping plus accepted-prediction integrity for Phase 6.';

create view public.request_segmentation_v4_activation_gate_status
with (security_invoker = true)
as
with target_contract as (
  select q.*
  ...
  select * from public.request_segmentation_v4_quality_summary
), per_segment as (
  select
  ...
  from public.request_segmentation_v4_segment_quality
), mapping as (
  select * from public.request_segmentation_v4_mapping_integrity
)
select
  tc.version as quality_gate_version,
  tc.taxonomy_version,
...
comment on view public.request_segmentation_v4_activation_gate_status is
  'Fail-closed technical Phase-6 gate, including exact model and research-contract integrity. Four pilot cases can never satisfy the 300/25 quality thresholds.';

create view public.request_segmentation_v4_activation_approval_status
with (security_invoker = true)
as
with active_approval as (
  ...
  where a.approval_scope = 'followup_pricing'
    and a.policy_version = 'nt_policy_v4_20260820_cx8_shadow'
    and a.taxonomy_version = 'nt_taxonomy_v2_20260819_cx8'
    and a.quality_gate_version = 'nt_quality_gate_v4_20260820_cx8'
...
select
  ...
  'nt_policy_v4_20260820_cx8_shadow'::text,
  'nt_taxonomy_v2_20260819_cx8'::text,
  'nt_quality_gate_v4_20260820_cx8'::text,
  null::text,
  null::text,
  null::timestamptz,
  null::timestamptz,
...
create view public.request_segmentation_v4_production_readiness
with (security_invoker = true)
as
select
  ...
  array_remove(
    g.technical_blocking_reasons || array[
      ...
      'phase6_evaluation_only_contract_requires_separate_global_rollout'
    ],
    null
  ) as blocking_reasons,
  g.active_segments,
...
from public.request_segmentation_v4_activation_gate_status g
cross join public.request_segmentation_v4_activation_approval_status a;

comment on view public.request_segmentation_v4_production_readiness is
  'Phase-6 is evaluation-only by construction. Even a future technical pass and approval cannot authorize follow-up or pricing without a separately approved global rollout contract.';

revoke all on table public.request_segmentation_v4_gold_evaluation from public, anon, authenticated, service_role;
revoke all on table public.request_segmentation_v4_confusion_matrix from public, anon, authenticated, service_role;
revoke all on table public.request_segmentation_v4_segment_quality from public, anon, authenticated, service_role;
revoke all on table public.request_segmentation_v4_quality_summary from public, anon, authenticated, service_role;
revoke all on table public.request_segmentation_v4_mapping_integrity from public, anon, authenticated, service_role;
revoke all on table public.request_segmentation_v4_activation_gate_status from public, anon, authenticated, service_role;
revoke all on table public.request_segmentation_v4_activation_approval_status from public, anon, authenticated, service_role;
revoke all on table public.request_segmentation_v4_production_readiness from public, anon, authenticated, service_role;

grant select on table public.request_segmentation_v4_gold_evaluation to service_role;
grant select on table public.request_segmentation_v4_confusion_matrix to service_role;
grant select on table public.request_segmentation_v4_segment_quality to service_role;
grant select on table public.request_segmentation_v4_quality_summary to service_role;
grant select on table public.request_segmentation_v4_mapping_integrity to service_role;
grant select on table public.request_segmentation_v4_activation_gate_status to service_role;
grant select on table public.request_segmentation_v4_activation_approval_status to service_role;
grant select on table public.request_segmentation_v4_production_readiness to service_role;

do $phase6_base_postcondition$
declare
...
begin
  ...
  where p.version = 'nt_policy_v4_20260820_cx8_shadow'
    ...
    and p.classifier_version = 'segment_classifier_v5_20260820_cx8'
    and p.prompt_version = 'segment_prompt_v5_20260820_cx8'
    and p.quality_gate_version = 'nt_quality_gate_v4_20260820_cx8';
  ...
  where n.nspname = 'public'
    and c.relkind = 'v'
    and c.relname in (
      'request_segmentation_v4_gold_evaluation',
      'request_segmentation_v4_confusion_matrix',
      'request_segmentation_v4_segment_quality',
      'request_segmentation_v4_quality_summary',
      'request_segmentation_v4_mapping_integrity',
      'request_segmentation_v4_activation_gate_status',
      'request_segmentation_v4_activation_approval_status',
      'request_segmentation_v4_production_readiness'
    )
  ...
  if v_gate_count <> 1
     ...
     or v_view_count <> 8
     or not public.neontrip_phase6_evaluation_contract_is_exact()
     or to_regprocedure('public.neontrip_phase6_evaluation_contract_is_exact()') is null
     or to_regprocedure('public.neontrip_phase6_redact_segmentation_text(text,integer,text[])') is null
     or to_regprocedure('public.neontrip_phase6_evaluation_research_context(uuid)') is null
     or to_regprocedure('public.neontrip_claim_request_segmentation_phase6_evaluation(integer,text,integer)') is null
     or to_regprocedure('public.neontrip_get_request_segmentation_phase6_evaluation_payload(uuid)') is null
    ...
    raise exception using
      errcode = '55000',
      message = 'phase6_base_postcondition_failed',
      detail = format(
...
$phase6_base_postcondition$;

-- The new 19-argument overload must be visible to PostgREST by its complete
