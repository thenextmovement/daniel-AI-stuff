...
with accepted_17track as (
  ...
  set status = 'resolved',
      ...
      metadata = i.metadata || jsonb_build_object(
        'resolved_by', '17track_accepted_direct_tracking_suppression',
        'resolved_reason', '17TRACK registration accepted; direct DHL/FedEx tracking path suppressed'
      )
  from accepted_17track a
  where i.shipment_id = a.shipment_id
    and i.incident_type = 'tracking_error'
    and i.status in ('open', 'acknowledged')
    and i.title ilike 'Tracking API Fehler:%konnte nicht abgefragt werden'
  returning i.shipment_id
)
update public.inbound_shipments s
set risk_level = case when s.status in ('delivered', 'closed') then 'closed' else 'watch' end,
    status_reason = 'tracking_registered:17track',
    metadata = (s.metadata - 'last_tracking_error') || jsonb_build_object(
      'tracking_provider', '17track',
      'direct_tracking_suppressed_at', now()
    ),
    updated_at = now()
where s.id in (select resolved.shipment_id from resolved)
  and not exists (
    select 1
    from public.inbound_incidents open_incident
    where open_incident.shipment_id = s.id
      and open_incident.status in ('open', 'acknowledged')
  );
