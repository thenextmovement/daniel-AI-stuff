-- Small, read-only relationship lookup for the NEONTRIP request acknowledgement.
--
-- Identity is deliberately exact-email only. NEONTRIP sources stay isolated from
-- RIESENOBJEKTE, which has its own intake, sender and data contract.
...
comment on function public.get_request_autoreply_relationship_context(text, text) is
  'Returns a bounded NEONTRIP-only relationship class for request acknowledgement copy. Exact normalized email only; no RIESENOBJEKTE history.';
