...
declare
  ...
  v_browser_manual_review boolean;
begin
  select * into v_case
  ...
  select exists (
    ...
    where j.case_id = v_case.id
      and j.status = 'manual_review'
  ) into v_browser_manual_review;

  if v_case.status not in ('manual_review', 'missing_data', 'ambiguous_match', 'conflicting_instructions', 'special_case')
    or (v_case.selected_dpd_product is not null and not v_browser_manual_review) then
    raise exception 'only a blocked case without DPD product or a fail-closed browser review can enqueue a review notification';
  end if;
...
comment on function public.arrival_labels_enqueue_review_notification(uuid, text, text, text, text, text)
  is 'Queues internal review mail for blocked pre-purchase cases or a browser job that failed closed in manual review.';
