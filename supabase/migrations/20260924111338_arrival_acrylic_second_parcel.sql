...
-- Extend the existing queues; one main parcel per arrival and one acrylic parcel per order.
alter table public.arrival_label_browser_purchase_jobs
  add column parcel_kind text not null default 'main',
  add column parent_purchase_job_id uuid references public.arrival_label_browser_purchase_jobs(id) on delete restrict,
  add column expected_primary_dpd_tracking text,
  add constraint arrival_browser_parcel_kind_check check (
    (parcel_kind = 'main' and parent_purchase_job_id is null and expected_primary_dpd_tracking is null)
    or (parcel_kind = 'acrylic_table_device' and parent_purchase_job_id is not null
      and parent_purchase_job_id <> id
      and (expected_primary_dpd_tracking is null or expected_primary_dpd_tracking ~ '^[0-9]{11,20}$'))
  ),
  drop constraint arrival_label_browser_purchase_jobs_case_id_key,
  add constraint arrival_browser_case_parcel_unique unique (case_id, parcel_kind);

create unique index arrival_browser_acrylic_order_unique
  on public.arrival_label_browser_purchase_jobs (shop_domain, shopify_order_numeric_id)
  where parcel_kind = 'acrylic_table_device';

alter table public.arrival_label_artifacts
  add column parcel_kind text not null default 'main'
    check (parcel_kind in ('main', 'acrylic_table_device')),
  add constraint arrival_artifact_delivery_note_main check (artifact_kind <> 'delivery_note_pdf' or parcel_kind = 'main'),
  drop constraint arrival_label_artifacts_case_kind_unique,
  add constraint arrival_artifacts_case_kind_parcel_unique unique (case_id, artifact_kind, parcel_kind);

-- Old clients omit the new optional capability and can only claim main parcels.
drop function public.arrival_labels_claim_browser_purchase(text, integer, timestamptz);
...
begin
  ...
  on conflict (case_id, parcel_kind) do nothing
  returning * into v_job;

  if not found then
    select * into v_job from public.arrival_label_browser_purchase_jobs where case_id = v_case.id and parcel_kind = 'main';
    ...
    -- Only a newly created primary purchase can plan the add-on. No historical backfill.
    if v_case.source_snapshot ->> 'acrylicTableDeviceRequired' = 'true' then
      insert into public.arrival_label_browser_purchase_jobs (
        case_id, parcel_kind, parent_purchase_job_id, idempotency_key,
        shop_domain, shopify_order_id, shopify_order_numeric_id, shopify_order_name, order_url,
        selected_dpd_product, easydpd_product_label, maximum_purchase_cents,
        incoming_dhl_tracking_number, incoming_dhl_last_six
      ) values (
        v_case.id, 'acrylic_table_device', v_job.id,
        'arrival-acrylic-parcel:' || v_job.shop_domain || ':' || v_numeric_order_id,
        v_job.shop_domain, v_job.shopify_order_id, v_numeric_order_id, v_job.shopify_order_name, v_job.order_url,
        v_job.selected_dpd_product, v_job.easydpd_product_label, v_job.maximum_purchase_cents,
        v_job.incoming_dhl_tracking_number, v_job.incoming_dhl_last_six
      )
      on conflict (shop_domain, shopify_order_numeric_id) where parcel_kind = 'acrylic_table_device' do nothing;
    end if;
...
create or replace function public.arrival_labels_claim_browser_purchase(
  ...
  p_acrylic_capable boolean default false
)
returns setof public.arrival_label_browser_purchase_jobs
language plpgsql
security invoker
...
begin
  ...
  where (parcel_kind = 'main' or p_acrylic_capable is true)
    and lease_owner = p_worker_id
    and status in ('claimed', 'validated')
    and lease_expires_at > p_now
    and (parcel_kind = 'main' or exists (
      select 1 from public.arrival_label_cases c
      join public.arrival_label_browser_purchase_jobs primary_job on primary_job.case_id = c.id and primary_job.parcel_kind = 'main'
      join public.arrival_label_print_jobs printed on printed.id = primary_job.print_job_id
      where c.id = arrival_label_browser_purchase_jobs.case_id
        and primary_job.id = arrival_label_browser_purchase_jobs.parent_purchase_job_id
        and primary_job.status = 'completed' and printed.status = 'printed'
        and primary_job.dpd_tracking_number = arrival_label_browser_purchase_jobs.expected_primary_dpd_tracking
        and c.existing_dpd_tracking = primary_job.dpd_tracking_number
        and c.status in ('completed', 'existing_label', 'pdf_processed') and c.manual_review_reason is null
        and c.source_snapshot ->> 'acrylicTableDeviceRequired' = 'true'
    ))
  ...
  where j.status in ('queued', 'claimed', 'validated', 'retryable_error')
    ...
    and (
      (j.parcel_kind = 'main' and c.status = 'label_planned' and c.existing_dpd_tracking is null)
      or (j.parcel_kind = 'acrylic_table_device' and p_acrylic_capable is true
        and c.status in ('completed', 'existing_label', 'pdf_processed')
        and c.manual_review_reason is null
        and c.source_snapshot ->> 'acrylicTableDeviceRequired' = 'true'
        and exists (
          select 1 from public.arrival_label_browser_purchase_jobs primary_job
          join public.arrival_label_print_jobs printed on printed.id = primary_job.print_job_id
          where primary_job.id = j.parent_purchase_job_id
            and primary_job.case_id = j.case_id and primary_job.parcel_kind = 'main'
            and primary_job.shopify_order_numeric_id = j.shopify_order_numeric_id
            and primary_job.status = 'completed' and printed.status = 'printed'
            and printed.cups_job_id is not null and printed.printed_at is not null
            and primary_job.dpd_tracking_number = c.existing_dpd_tracking
        ))
    )
  ...
  update public.arrival_label_browser_purchase_jobs
  set status = 'claimed',
      expected_primary_dpd_tracking = case when parcel_kind = 'acrylic_table_device'
        then (select primary_job.dpd_tracking_number from public.arrival_label_browser_purchase_jobs primary_job where primary_job.id = v_job.parent_purchase_job_id)
        else null end,
      attempts = attempts + 1,
...
begin
  ...
  if v_job.parcel_kind = 'acrylic_table_device' and p_result in ('validated', 'dispatching') then
    if not exists (
      select 1 from public.arrival_label_browser_purchase_jobs primary_job
      join public.arrival_label_print_jobs printed on printed.id = primary_job.print_job_id
      join public.arrival_label_cases c on c.id = v_job.case_id
      where primary_job.id = v_job.parent_purchase_job_id and primary_job.status = 'completed'
        and printed.status = 'printed' and printed.cups_job_id is not null
        and primary_job.dpd_tracking_number = v_job.expected_primary_dpd_tracking
        and c.existing_dpd_tracking = v_job.expected_primary_dpd_tracking
        and c.status in ('completed', 'existing_label', 'pdf_processed') and c.manual_review_reason is null
        and c.source_snapshot ->> 'acrylicTableDeviceRequired' = 'true'
    ) then raise exception 'additional parcel primary print or case approval is missing'; end if;
  end if;
  if p_result = 'purchased' then
    if v_job.parcel_kind = 'acrylic_table_device' and (
      v_job.expected_primary_dpd_tracking is null
      or coalesce(p_dpd_tracking_number, v_job.dpd_tracking_number) = v_job.expected_primary_dpd_tracking
  ...
  if p_result = 'completed' then
    ...
    if not exists (
      ...
      join public.arrival_label_artifacts a on a.id = p.artifact_id and a.parcel_kind = v_job.parcel_kind
...
begin
  ...
  if v_original.id is null or v_annotated.id is null or v_preview.id is null
    ...
    or coalesce(v_annotated.qa_result ->> 'overlayText', '') <> (case when v_job.parcel_kind = 'acrylic_table_device' then 'Acryl LED-Tischgerät' else v_job.incoming_dhl_last_six end)
...
begin
  ...
  if v_artifact.artifact_kind = 'annotated_pdf' then
    ...
    if v_artifact.parcel_kind = 'acrylic_table_device' and not exists (
      select 1 from public.arrival_label_browser_purchase_jobs extra
      join public.arrival_label_browser_purchase_jobs primary_job on primary_job.id = extra.parent_purchase_job_id
      join public.arrival_label_print_jobs printed on printed.id = primary_job.print_job_id
      where extra.case_id = p_case_id and extra.parcel_kind = 'acrylic_table_device'
        and extra.status in ('artifact_uploaded', 'completed')
        and extra.annotated_pdf_sha256 = v_artifact.sha256
        and printed.status = 'printed' and printed.cups_job_id is not null
        and extra.expected_primary_dpd_tracking = primary_job.dpd_tracking_number
    ) then raise exception 'additional parcel has no independent purchase and primary print proof'; end if;
    if (v_case.status not in ('pdf_processed', 'completed')
        and not (v_artifact.parcel_kind = 'acrylic_table_device' and v_case.status = 'existing_label'))
      or p_printer_key <> v_config.printer_key then
...
begin
  ...
  update public.arrival_label_browser_purchase_jobs
  set status = 'manual_review',
      dpd_tracking_number = case when v_job.parcel_kind = 'main' then coalesce(p_existing_dpd_tracking, dpd_tracking_number) else dpd_tracking_number end,
      lease_owner = null,
      lease_expires_at = null,
  ...
  update public.arrival_label_cases
  set status = 'manual_review',
      existing_dpd_tracking = case when v_job.parcel_kind = 'main' then coalesce(p_existing_dpd_tracking, existing_dpd_tracking) else existing_dpd_tracking end,
...
begin
  ...
  with eligible as (
    ...
    where j.id = p_print_job_id
      and not exists (
        select 1 from public.arrival_label_browser_purchase_jobs extra
        left join public.arrival_label_print_jobs extra_print on extra_print.id = extra.print_job_id
        where extra.case_id = c.id and extra.parcel_kind = 'acrylic_table_device'
          and (extra.status <> 'completed' or extra_print.status is distinct from 'printed' or extra_print.cups_job_id is null)
      )
...
$$;
revoke execute on function public.arrival_labels_claim_browser_purchase(text, integer, timestamptz, boolean) from public, anon, authenticated;
grant execute on function public.arrival_labels_claim_browser_purchase(text, integer, timestamptz, boolean) to service_role;
