-- Aggregate-only, PII-free prechange snapshot for the Phase-6 evaluation lane.
...
select
  ...
  )::integer as phase6_activation_approvals
...
where n.nspname = 'public'
  and p.proname in (
    'neontrip_phase6_evaluation_contract_is_exact',
...
select
  ...
  count(*) filter (
    where pg_get_function_identity_arguments(p.oid)
      like '%p_accepted_by text, p_research_contract text'
  )::integer as phase6_record_19_argument_overload
from pg_proc p
join pg_namespace n on n.oid = p.pronamespace
where n.nspname = 'public'
