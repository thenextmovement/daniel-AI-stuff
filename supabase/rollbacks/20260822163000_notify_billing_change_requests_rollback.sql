...
begin
  if exists (
    select 1 from public.billing_jobs
    where job_type='NOTIFY_CHANGE_REQUEST' and status='PROCESSING'
  ) then
    raise exception 'NOTIFY_CHANGE_REQUEST_JOB_PROCESSING';
  end if;
  delete from public.billing_jobs where job_type='NOTIFY_CHANGE_REQUEST';
end;
$$;
