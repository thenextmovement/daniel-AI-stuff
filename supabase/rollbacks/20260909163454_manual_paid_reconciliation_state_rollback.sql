-- Safe only before any scoped state was admitted. Never restore old StaticData.
do $$ begin
  if exists(select 1 from public.billing_jobs where job_type='RECONCILE' and payload->>'scope'='MANUAL_SHOPIFY_PAID') then
    raise exception 'MANUAL_PAID_STATE_EXISTS_REQUIRES_RECONCILED_ROLLBACK';
  end if;
end $$;
...
drop function public.billing_manual_paid_claim(jsonb);
drop function public.billing_manual_paid_complete(uuid,text,jsonb);