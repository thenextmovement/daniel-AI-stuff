-- History-preserving operational stop for Phase 6 after staging/runtime.
...
do $phase6_operational_stop_precondition$
declare
  v_processing integer;
  v_history_hash text;
begin
  if not public.neontrip_phase6_evaluation_contract_is_exact() then
    raise exception 'phase6_operational_stop_requires_exact_inactive_evaluation_contract';
  end if;
  ...
  if v_processing <> 0 then
    raise exception 'phase6_operational_stop_requires_zero_processing_jobs: %', v_processing;
  end if;

  if to_regprocedure('public.neontrip_claim_request_segmentation_phase6_evaluation(integer,text,integer)') is null
     or to_regprocedure('public.neontrip_get_request_segmentation_phase6_evaluation_payload(uuid)') is null
     or to_regprocedure('public.neontrip_record_request_segment_classification(uuid,uuid,text,text,text,numeric,text,text,text[],jsonb,jsonb,jsonb,text[],text,text,text,text,text,text)') is null then
    raise exception 'phase6_operational_stop_rpc_missing';
  end if;
  ...
  select md5(concat_ws('|',
    ...
    coalesce((
      ...
      where c.summary_json->>'classifier_version' = 'segment_classifier_v5_20260820_cx8'
    ), '')
  )) into v_history_hash;

  perform set_config('neontrip.phase6_history_hash', v_history_hash, true);
end;
$phase6_operational_stop_precondition$;

revoke execute on function public.neontrip_claim_request_segmentation_phase6_evaluation(integer, text, integer)
  from service_role;
revoke execute on function public.neontrip_get_request_segmentation_phase6_evaluation_payload(uuid)
  from service_role;
...
do $phase6_operational_stop_postcondition$
declare
  v_history_hash text;
begin
  if has_function_privilege(
       'service_role',
       'public.neontrip_claim_request_segmentation_phase6_evaluation(integer,text,integer)',
       'EXECUTE'
     )
     or has_function_privilege(
       'service_role',
       'public.neontrip_get_request_segmentation_phase6_evaluation_payload(uuid)',
       'EXECUTE'
     )
     or has_function_privilege(
       'service_role',
    ...
    raise exception 'phase6_operational_stop_execute_revoke_failed';
  end if;
  ...
  if v_history_hash is distinct from current_setting('neontrip.phase6_history_hash', true)
     or not public.neontrip_phase6_evaluation_contract_is_exact() then
    raise exception 'phase6_operational_stop_history_or_contract_changed';
  end if;
end;
$phase6_operational_stop_postcondition$;
