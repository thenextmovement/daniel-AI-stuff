-- Restore the pre-change read-only RPC; retain all jobs and customer records.
-- Return the persisted request product together with attachment context.
...
comment on function public.get_request_autoreply_relationship_context(text, text) is
  'Returns bounded NEONTRIP relationship, persisted attachment state, and request product. Exact normalized email only; no RIESENOBJEKTE history.';
