-- NEONTRIP Phase-5 exact pre-runtime rollback.
-- This destructive schema/config cleanup is permitted only before any v4 job,
-- classification, cache entry, or approval exists. It restores the current
-- pre-Phase-5 live function bodies, including the human-Gold customer_type
-- disagreement repair; it never restores the removed NT-8/NT-9 Gold veto.
...
do $phase5_base_rollback_precondition$
declare
  v_old_active_policy integer;
  v_old_active_quality integer;
  v_candidate_inactive_policy integer;
  v_candidate_inactive_quality integer;
  ...
  v_candidate_cache_rows integer;
...
begin
  ...
  select count(*) into v_candidate_inactive_policy
  from public.segment_policy_versions p
  ...
  select count(*) into v_candidate_inactive_quality
  from public.segment_quality_gate_versions q
  ...
  select count(*) into v_candidate_cache_rows
  ...
  where a.policy_version = 'nt_policy_v3_20260820_cx8_shadow'
     or a.quality_gate_version = 'nt_quality_gate_v3_20260820_cx8';

  if v_old_active_policy <> 1
     or v_old_active_quality <> 1
     or v_candidate_inactive_policy <> 1
     or v_candidate_inactive_quality <> 1
     or v_global_active_policy_count <> 1
     or v_global_active_quality_count <> 1 then
    raise exception using
      errcode = '55000',
      message = 'phase5_base_rollback_requires_old_active_candidate_inactive',
      detail = format(
        'old_policy=%s old_quality=%s candidate_policy=%s candidate_quality=%s active_policies=%s active_gates=%s',
        v_old_active_policy, v_old_active_quality,
        v_candidate_inactive_policy, v_candidate_inactive_quality,
        v_global_active_policy_count, v_global_active_quality_count
      );
  end if;

  if v_candidate_jobs <> 0
     or v_candidate_classifications <> 0
     or v_candidate_cache_rows <> 0
     or v_candidate_approvals <> 0 then
    raise exception using
      errcode = '55000',
      message = 'phase5_base_rollback_requires_zero_candidate_runtime_rows',
      detail = format(
        'jobs=%s classifications=%s cache=%s approvals=%s',
        v_candidate_jobs, v_candidate_classifications,
        v_candidate_cache_rows, v_candidate_approvals
      );
  end if;
end;
$phase5_base_rollback_precondition$;

drop view public.request_segmentation_v3_production_readiness;
drop view public.request_segmentation_v3_activation_approval_status;
drop view public.request_segmentation_v3_activation_gate_status;
drop view public.request_segmentation_v3_mapping_integrity;
drop view public.request_segmentation_v3_quality_summary;
drop view public.request_segmentation_v3_segment_quality;
drop view public.request_segmentation_v3_confusion_matrix;
drop view public.request_segmentation_v3_gold_evaluation;
...
drop function public.neontrip_get_request_segmentation_gold_target_contract();

delete from public.segment_policy_rules
where policy_version = 'nt_policy_v3_20260820_cx8_shadow';
