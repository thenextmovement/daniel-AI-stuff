-- Disable post-delivery and repeat-business claims while preserving existing audit
-- evidence and the additive table constraint. Deactivate both dependent n8n workflows
...
comment on function public.claim_customer_communication_draft(text, text, text, text, integer) is
  'Claims one customer draft job atomically. Claims design-reminder, win-back, and ActiveCampaign auto-reply drafts. Post-delivery and repeat-business claims are disabled after rollback; retained rows remain audit evidence.';
