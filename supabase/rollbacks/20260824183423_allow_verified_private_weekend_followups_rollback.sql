-- Roll back verified-private weekend follow-ups to the prior weekday-only
-- cadence contract. This changes no queue rows and sends no messages.

do $private_weekend_rollback_precondition$
declare
...
begin
  if cadence_oid is null
     or md5(pg_get_functiondef(cadence_oid)) <> '7b81bf6b3fa457e4cc0974b2f948ae9d'
     or claim_oid is null
     or md5(pg_get_functiondef(claim_oid)) <> 'fd57481ae37fb33d31aeeff69786443a'
     or complete_oid is null
     or md5(pg_get_functiondef(complete_oid)) <> '2b51b4c6da57c441199909bde1bc5204' then
    raise exception using errcode = '55000',
      message = 'private_weekend_rollback_function_drift';
  end if;
  if exists (
    select 1 from public.followup_delivery_attempts where status = 'processing'
  ) then
    raise exception using errcode = '55000',
      message = 'private_weekend_rollback_requires_idle_delivery_loop';
  end if;
end;
$private_weekend_rollback_precondition$;
...
drop function public.neontrip_followup_delivery_window_allowed(jsonb,timestamptz);
drop function public.neontrip_followup_calendar_slot(timestamptz,integer,text);
...
do $private_weekend_rollback_postcondition$
begin
  if md5(pg_get_functiondef(to_regprocedure(
       'public.neontrip_get_followup_queue_cadence_decision(uuid)'
     ))) <> '2950862699361ab230501fe22f41754c'
     or md5(pg_get_functiondef(to_regprocedure(
       'public.claim_followup_delivery_candidate(text,integer)'
     ))) <> '2659f7d2e4994ba81578da05b7830f45'
     or md5(pg_get_functiondef(to_regprocedure(
       'public.complete_followup_delivery(uuid,uuid,text,text,text,text)'
     ))) <> '7e1dfd478d36c22ea440a1060ef8d971'
     or to_regprocedure(
    ...
    raise exception using errcode = '55000',
      message = 'private_weekend_rollback_postcondition_failed';
  end if;
end;
$private_weekend_rollback_postcondition$;
