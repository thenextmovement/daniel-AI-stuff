...
begin
  if exists (
    ...
    where projection_type = 'trello_card_attachment_v1'
  ) then
    raise exception
      'Refusing full rollback while Trello attachment outbox rows exist';
  end if;
end;
$guard$;
...
CREATE OR REPLACE FUNCTION public.ro_claim_projection_outbox_v2(p_worker_id text, p_projection_types text[], p_limit integer DEFAULT 1, p_lease_seconds integer DEFAULT 180)
 RETURNS SETOF ro_projection_outbox
 LANGUAGE plpgsql
...
CREATE OR REPLACE FUNCTION public.ro_complete_projection_outbox_v1(p_outbox_id uuid, p_worker_id text, p_lease_token uuid, p_external_id text, p_external_url text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.ro_record_first_party_attachment_v1(p_request_id text, p_submission_id uuid, p_attachment_index integer, p_storage_bucket text, p_storage_path text, p_original_file_name text, p_mime_type text, p_size_bytes bigint, p_sha256 text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO 'public'
AS $function$
...
drop function public.ro_enqueue_trello_attachment_projection_v1(uuid);
