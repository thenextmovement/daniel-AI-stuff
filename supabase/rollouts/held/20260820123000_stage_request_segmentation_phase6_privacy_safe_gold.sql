-- HOLD: do not apply until the exact n8n-v5 graph and both Phase-6 RPC
-- overloads have been published, read back and approved. This artifact only
...
begin
  if not public.neontrip_phase6_evaluation_contract_is_exact() then
    raise exception 'phase6_stage_requires_exact_inactive_evaluation_contract';
  end if;
  ...
  if v_current_gold_count <> 4
    ...
    raise exception using
      errcode = '55000',
      message = 'phase6_stage_requires_four_current_gold_and_pristine_lane',
      detail = format(
  ...
  perform set_config('neontrip.phase6_master_hash', v_master_hash, true);
end;
$phase6_stage_precondition$;

do $phase6_stage_four_current_gold_jobs$
declare
  v_inserted integer;
begin
  with current_gold as (
    ...
    select
      ...
      jsonb_build_object(
        ...
        'staged_by', 'phase6-held-four-current-gold',
        'staged_at', now()
      ),
  ...
  if v_inserted <> 4 then
    raise exception 'phase6_stage_did_not_insert_exactly_four_jobs: %', v_inserted;
  end if;
end;
$phase6_stage_four_current_gold_jobs$;

do $phase6_stage_postcondition$
declare
  v_job_count integer;
...
begin
  select
    ...
    count(*) filter (
      where j.source <> 'gold_re_evaluation_phase6'
         ...
         or j.metadata->>'policy_version' <> 'nt_policy_v4_20260820_cx8_shadow'
         or j.metadata->>'quality_gate_version' <> 'nt_quality_gate_v4_20260820_cx8'
         or j.metadata->>'research_contract' <> 'segment_research_v1_20260820_cx8'
         or j.metadata->>'validator_version' <> 'n8n_cx8_validator_v2'
  ...
  if v_job_count <> 4
     ...
     or v_candidate_classifications <> 0
     or v_candidate_cache <> 0
     or v_master_hash is distinct from current_setting('neontrip.phase6_master_hash', true)
     or not public.neontrip_phase6_evaluation_contract_is_exact() then
    raise exception using
      errcode = '55000',
      message = 'phase6_stage_postcondition_failed',
      detail = format(
        'jobs=%s invalid=%s classifications=%s cache=%s master_unchanged=%s',
        v_job_count, v_invalid_job_count, v_candidate_classifications,
        v_candidate_cache,
        v_master_hash is not distinct from current_setting('neontrip.phase6_master_hash', true)
      );
  end if;
end;
$phase6_stage_postcondition$;
