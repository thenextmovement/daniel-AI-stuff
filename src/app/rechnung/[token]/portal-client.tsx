...
export function BillingPortalClient({ token }: { token: string }) {
  ...
  const [form, setForm] = useState({ company: "", street: "", zip: "", city: "", country: "", vatId: "", invoiceEmail: "" });
  ...
  async function load() {
    ...
    if (payload.ok && payload.billingCase) {
      ...
      setForm({ company: String(address.company || ""), street: String(address.street || ""), zip: String(address.zip || ""), city: String(address.city || ""), country: String(address.country || ""), vatId: String(payload.billingCase.vat_id || ""), invoiceEmail: String(address.invoiceEmail || payload.billingCase.customer_email || "") });
    }
  }
  async function submit(event: FormEvent) {
    event.preventDefault(); setSending(true); setMessage(null);
    const response = await fetch(`/api/rechnung/${encodeURIComponent(token)}`, { method: "POST", headers: { "Content-Type": "application/json", "Idempotency-Key": crypto.randomUUID() }, body: JSON.stringify({ billingAddress: { company: form.company, street: form.street, zip: form.zip, city: form.city, country: form.country }, vatId: form.vatId, invoiceEmail: form.invoiceEmail, requesterEmail: form.invoiceEmail }) });
