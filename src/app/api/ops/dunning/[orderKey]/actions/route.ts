...
type ActionBody = {
  action: "preview_next_stage" | "send_next_stage";
  confirmation?: string;
...
function parseBody(value: unknown): ActionBody | null {
  ...
  if (action !== "preview_next_stage" && action !== "send_next_stage") return null;
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ orderKey: string }> }) {
  ...
  try {
    ...
    const status = message === "DUNNING_SEND_NOT_ENABLED" ? 503 : message === "DUNNING_DUPLICATE_OR_STALE" || message === "DUNNING_SEND_BLOCKED" ? 409 : expected ? 502 : 500;
