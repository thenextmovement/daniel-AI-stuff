...
import { OpsOfferApiError, sendOfferUpdateMail, type OpsOfferSendInput } from "@/lib/ops/offers";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ offerId: string }> }) {
  ...
  try {
    ...
    const result = await sendOfferUpdateMail(decodeURIComponent(offerId), body);
