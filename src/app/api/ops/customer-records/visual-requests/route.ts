...
import { requestOpsVisual, type OpsVisualRequestInput } from "@/lib/ops/visual-requests";
...
function failureResponse(error: unknown) {
  ...
  console.error("ops visual request route failed", error);
...
export async function POST(request: NextRequest) {
  ...
  try {
    const body = (await request.json()) as OpsVisualRequestInput;
    const result = await requestOpsVisual(body);
