...
function hasShippingAutomationAccess(request: NextRequest, bodyToken?: string | null) {
  const expected = String(process.env.SHIPPING_AGENT_API_TOKEN || "").trim();
...
function getAutomationActor(request: NextRequest, operatorName?: string | null) {
  return {
...
export async function POST(request: NextRequest) {
  ...
  const automationAllowed = hasShippingAutomationAccess(request, body?.agentToken || null);
  if (!opsActor && automationAllowed && !isShippingAutomationAction(body?.action)) return forbidden();

  const actor =
    opsActor ||
    (automationAllowed && isShippingAutomationAction(body?.action)
