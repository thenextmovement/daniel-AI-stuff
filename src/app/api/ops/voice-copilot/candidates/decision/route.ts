...
import { decideVoiceKnowledgeCandidate, isVoiceKnowledgeEnabled } from "@/lib/ops/voice-knowledge";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await decideVoiceKnowledgeCandidate(await request.json());
    return NextResponse.json({ ok: true, result });
  } catch (error) {
    return voiceCopilotApiFailure(error, "candidate-decision");
  }
}
