...
export async function POST(request: NextRequest) {
  ...
  const allowed = new Set(["CREATE_PROFORMA", "CREATE_INVOICE", "CREATE_CREDIT", "CREATE_CANCELLATION", "VOID_PROFORMA", "PROJECT_PAYMENT_SHOPIFY", "PROJECT_PAYMENT_EASYBILL", "SEND_CUSTOMER_DOCUMENT", "VERIFY_VAT", "SYNC_SHOPIFY_TAX", "RECONCILE"]);
  const jobTypes = Array.isArray(body.jobTypes) ? [...new Set(body.jobTypes.map(String).filter((value) => allowed.has(value)))].slice(0, 10) : [];
