...
export function VoicePlatformPanel({ operatorName }: { operatorName: string }) {
  ...
  const latestAttemptByTarget = useMemo(() => {
    const map = new Map<string, Row>();
    for (const attempt of dashboard?.attempts || []) if (!map.has(text(attempt.target_id))) map.set(text(attempt.target_id), attempt);
    return map;
  }, [dashboard]);
  ...
  return <div className="grid gap-5 max-sm:px-6 xl:pr-52">
    ...
    <section className="grid gap-4 rounded-lg border border-stone-200 bg-white p-5">
      ...
      <label className="grid gap-2 text-sm">Auftrag und Zusatzinformationen<textarea value={callBrief} maxLength={1200} onChange={event=>setCallBrief(event.target.value)} placeholder="Zum Beispiel: Lieferadresse bestätigen lassen." className="rounded-lg border border-stone-200 p-3"/></label>
      <label className="grid gap-2 text-sm">Kundenvorgang für internen Test (optional)<input value={testContext} onChange={event=>setTestContext(event.target.value)} className="rounded-lg border border-stone-200 p-3"/></label>
      <label className="flex gap-2 text-sm"><input type="checkbox" checked={transcriptConsent} onChange={event=>setTranscriptConsent(event.target.checked)}/>Einwilligung zur Transkription und Speicherung liegt vor.</label>
      <button type="button" disabled={busy || !targetCampaign || !targetConsent || !targetRequest || !targetPhone || !transcriptConsent} onClick={() => action("add_target", { campaignId: targetCampaign, consentId: targetConsent, requestId: targetRequest, phone: targetPhone, callBrief, contextRequestId: testContext || null, transcriptConsent })} className="inline-flex min-h-11 w-fit items-center gap-2 rounded-lg bg-stone-950 px-4 text-sm font-semibold text-white disabled:opacity-50"><Plus className="h-4 w-4" /> Call-Ziel anlegen</button>
      <div className="divide-y divide-stone-100 border-t border-stone-200">{(dashboard.targets || []).map((entry) => { const latestAttempt = latestAttemptByTarget.get(text(entry.id)); const needsProviderReview = entry.status === "blocked" && entry.blocked_reason === "manual_provider_reconciliation_required"; return <div key={text(entry.id)} className="grid gap-3 py-3 text-sm lg:grid-cols-[1fr_auto] lg:items-center"><div><span>{badge(entry.status)} <strong>{text(entry.request_id)}</strong> · {text(entry.phone_e164)}</span><p className="mt-1 text-xs text-stone-500">Versuche {text(entry.attempt_count)}{entry.blocked_reason ? ` · ${text(entry.blocked_reason)}` : ""}{latestAttempt?.provider_call_id ? ` · Provider ${text(latestAttempt.provider_call_id)}` : ""}</p></div>{needsProviderReview ? <div className="flex flex-wrap gap-2"><button type="button" disabled={busy} onClick={() => action("resolve_provider_uncertainty", { targetId: entry.id, resolution: "confirmed_no_call", confirmation: "PROVIDER GEPRUEFT KEIN CALL" }, "Twilio-Protokoll geprüft und sicher bestätigt, dass kein Anruf stattgefunden hat? Das Ziel wird einmalig erneut freigegeben.")} className="min-h-9 rounded-lg border border-amber-300 px-3 text-xs font-semibold text-amber-900">Kein Call, erneut versuchen</button><button type="button" disabled={busy} onClick={() => action("resolve_provider_uncertainty", { targetId: entry.id, resolution: "close_without_retry", confirmation: "PROVIDER FALL SCHLIESSEN" }, "Provider-Fall ohne weiteren Anruf endgültig schließen?")} className="min-h-9 rounded-lg border border-stone-300 px-3 text-xs font-semibold">Ohne Retry schließen</button></div> : null}</div>; })}</div>
    </section>
