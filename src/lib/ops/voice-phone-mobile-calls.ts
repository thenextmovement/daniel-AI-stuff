...
import {configuredMobileCalling} from "./voice-phone-identity";
const FIELDS="id,call_id,staff_id,device_id,mobile_link_id,phone,state,provider_call_sid,claimed_at,confirmed_at,expires_at,ended_at,provider_ended_at,cleanup_pending,updated_at";
export async function runtimeMobileCallAction(input:Record<string,unknown>){
 ...
 if(input.action==="get")return {leg,call:await getRuntimePhoneCall(leg.call_id),dial:false,join:false,closeCall:false};
 ...
 if(["claim","prompt","confirm"].includes(input.kind)&&(!configuredMobileCalling()||
