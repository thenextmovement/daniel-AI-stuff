import { createCustomerInternalTask, listCustomerInternalTasks, type UpdateActor } from "@/lib/ops/customer-records";
...
export type ShippingAutomationAction = "upsert_shipment" | "record_event" | "evaluate_shipment";

export function isShippingAutomationAction(action: string | null | undefined): action is ShippingAutomationAction {
  return action === "upsert_shipment" || action === "record_event" || action === "evaluate_shipment";
}
...
function isUniqueConstraintError(error: unknown, constraint: string) {
  ...
  return (message.includes("23505") || message.includes("duplicate key")) && message.includes(constraint);
}
...
async function insertShippingAudit(input: {
  ...
  try {
    ...
    if (isUniqueConstraintError(error, "shipping_audit_log_idempotency_key_key")) return;
    throw error;
  }
}
...
export async function updateShippingIncidentStatus(
  ...
  await insertShippingAudit({
    ...
    idempotencyKey: `shipping:${incident.id}:status:${status}`,
    actor,
  });
  return updated;
}

export async function createShippingIncidentTask(incidentId: string, actor?: UpdateActor | null) {
  const { incident, shipment } = await getIncidentWithShipment(incidentId);
  if (incident.active_task_id) {
    return { incident: mapIncident(incident), taskId: incident.active_task_id, created: false };
  }

  const existingTasks = await listCustomerInternalTasks({ includeDone: false, requestId: incident.request_id || shipment.request_id });
  const existing = existingTasks.tasks.find((task) =>
    (task.sourceType === "shipping_incident" && task.sourceId === incident.id) ||
    task.description?.includes(`Shipping Incident: ${incident.id}`)
  );
  if (existing) {
