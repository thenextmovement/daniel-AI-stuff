...
const FIELDS="id,device_id,staff_id,phone,state,agent_call_sid,customer_call_sid,conference_sid,customer_dispatch,agent_joined,customer_joined,created_at,updated_at,ended_at,cleanup_pending";
...
export function publicPhoneCall(call:PhoneCallRecord) {
 return {id:call.id,state:call.state,phone:call.phone,startedAt:call.created_at,endedAt:call.ended_at,connected:call.customer_joined && !call.ended_at,cleanupPending:call.cleanup_pending,isTest:true};
}
