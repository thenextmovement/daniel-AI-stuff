...
export async function recordDeliveryOutcome(input:{deliveryId?:unknown;outcome?:"sent"|"retryable_failure"|"terminal_failure";providerMessageId?:unknown;errorCode?:unknown;errorSummary?:unknown;workflowExecutionId?:unknown}) {
...
export async function ingestReply(input:{internetMessageId?:unknown;conversationId?:unknown;senderEmail?:unknown;receivedAt?:unknown;subject?:unknown;bodyExcerpt?:unknown;attachments?:unknown;requestId?:unknown;extraction?:unknown}) {
  ...
  const requestId=String(input.requestId||"")||null; const extraction=input.extraction===undefined?null:validateOfferExtraction(input.extraction);
  const replyRows=await supabaseRequest<Array<{id:string}>>("eu_supplier_replies",{method:"POST",body:JSON.stringify({internet_message_id:required(input.internetMessageId,"internetMessageId"),conversation_id:String(input.conversationId||"")||null,sender_email:senderEmail,sender_domain:emailDomain(senderEmail),received_at:required(input.receivedAt,"receivedAt"),subject:String(input.subject||"").slice(0,500)||null,body_excerpt:String(input.bodyExcerpt||"").slice(0,2000)||null,attachment_manifest:Array.isArray(input.attachments)?input.attachments:[],request_id:requestId,organization_id:match.organization?.id||null,match_status:match.matchStatus,extraction_status:extraction&&requestId&&match.organization?(extraction.confidence>=0.8?"validated":"needs_review"):"pending",extraction_confidence:extraction?.confidence??null,raw_extraction:extraction}),headers:{Prefer:"resolution=ignore-duplicates,return=representation"}},{on_conflict:"internet_message_id"});
