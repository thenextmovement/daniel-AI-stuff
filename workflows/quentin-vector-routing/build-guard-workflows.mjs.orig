...
const output = [];
const seen = new Set();
for (const action of moves.sort((a,b) => Date.parse(a.date || '') - Date.parse(b.date || ''))) {
  ...
  if (!Number.isFinite(time) || now - time < delayMs || now - time > horizonMs || !action.id || seen.has(action.id)) continue;
  seen.add(action.id);
