...
return [{json:{internetMessageId:String(message.internetMessageId),conversationId:String(message.conversationId||''),senderEmail:String(message.senderEmail),receivedAt:String(message.receivedAt),subject:String(message.subject||'').slice(0,500),bodyExcerpt:String(message.bodyText||'').slice(0,2000),attachments:safeAttachments,aiAttachments}}];` }),
    node("build-ai", "Build Untrusted Extraction Input", "n8n-nodes-base.code", [1100,0], { jsCode: `const {aiAttachments,...source}=$json; const content=[{type:'input_text',text:'SUBJECT\\n'+source.subject+'\\n\\nEMAIL\\n'+source.bodyExcerpt}];
...
return [{json:{source,request:{model:String($env.EU_SUPPLIER_OPENAI_MODEL||'gpt-4.1-mini'),instructions,input:[{role:'user',content}],text:{format:{type:'json_schema',name:'eu_supplier_offer',strict:true,schema}}}}}];` }),
    node("extract-ai", "Extract Offer as Strict JSON", "n8n-nodes-base.httpRequest", [1320,0], { method: "POST", url: "https://api.openai.com/v1/responses", authentication: "genericCredentialType", genericAuthType: "httpHeaderAuth", sendBody: true, specifyBody: "json", jsonBody: "={{ $json.request }}", options: { timeout: 60000, response: { response: { fullResponse: true, responseFormat: "json" } } } }, { credentials: { httpHeaderAuth: { id: "CONFIGURE_EU_SUPPLIER_OPENAI", name: "OpenAI | EU Supplier Extraction" } }, retryOnFail: false, onError: "continueErrorOutput" }),
    node("parse-ai", "Parse AI JSON Only", "n8n-nodes-base.code", [1540,-100], { jsCode: `const response=$json.body ?? $json; const text=response.output_text ?? response.output?.flatMap(x=>x.content||[]).find(x=>x.type==='output_text')?.text; if(typeof text!=='string') throw new Error('ai_json_missing'); const extraction=JSON.parse(text); return [{json:{...$('Build Untrusted Extraction Input').item.json.source,extraction}}];` }),
    node("manual-review", "Prepare Manual Review", "n8n-nodes-base.code", [1540,120], { jsCode: `return [{json:{...$('Build Untrusted Extraction Input').item.json.source,extraction:undefined,aiError:String($json.error?.message||$json.message||'ai_extraction_failed').slice(0,500)}}];` }),
    node("sign-ingest", "Sign Reply Ingest", "n8n-nodes-base.code", [1760,-100], { jsCode: signCode(`{ action: 'ingest_reply', ...$json }`) }), apiCall("ingest", "Ingest Reply and Offer", [1980,-100]),
    node("sign-review", "Sign Manual Review Ingest", "n8n-nodes-base.code", [1760,120], { jsCode: signCode(`{ action: 'ingest_reply', ...$json }`) }), apiCall("ingest-review", "Ingest Reply for Manual Review", [1980,120]),
    node("respond-ok", "Acknowledge Reply", "n8n-nodes-base.respondToWebhook", [2200,-100], { respondWith: "json", responseBody: "={{ { ok: true, result: $json.body ?? $json } }}", options: {} }),
    node("respond-review", "Acknowledge Manual Review", "n8n-nodes-base.respondToWebhook", [2200,120], { respondWith: "json", responseCode: 202, responseBody: "={{ { ok: true, reviewRequired: true } }}", options: {} }),
  ],
  connections: { "Graph Reply Webhook": { main: [[edge("Validate Graph Notification")]] }, "Validate Graph Notification": { main: [[edge("Fetch Immutable Graph Message")]] }, "Fetch Immutable Graph Message": { main: [[edge("Normalize Graph Message")]] }, "Normalize Graph Message": { main: [[edge("Validate Bounded Reply")]] }, "Validate Bounded Reply": { main: [[edge("Build Untrusted Extraction Input")]] }, "Build Untrusted Extraction Input": { main: [[edge("Extract Offer as Strict JSON")]] }, "Extract Offer as Strict JSON": { main: [[edge("Parse AI JSON Only")], [edge("Prepare Manual Review")]] }, "Parse AI JSON Only": { main: [[edge("Sign Reply Ingest")]] }, "Prepare Manual Review": { main: [[edge("Sign Manual Review Ingest")]] }, "Sign Reply Ingest": { main: [[edge("Ingest Reply and Offer")]] }, "Ingest Reply and Offer": { main: [[edge("Acknowledge Reply")]] }, "Sign Manual Review Ingest": { main: [[edge("Ingest Reply for Manual Review")]] }, "Ingest Reply for Manual Review": { main: [[edge("Acknowledge Manual Review")]] } },
};
