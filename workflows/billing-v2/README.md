...
The separate delivery worker sends only an already finalized Easybill document. Its pilot queue and worker both fail closed unless the invoice email is exactly `rahim.hedayati@icloud.com` or `info@riesenobjekte.de`. The first Pro-forma is sent as one combined order-confirmation/Pro-forma email; later revisions, invoices, credits and cancellations use explicit document-specific subjects. The Easybill PDF is attached, the permanent billing portal and AGB are linked, and retries reload `last_postbox_id` before sending to prevent duplicate mail. The allowlist must be deliberately replaced in a separately reviewed cutover before any real-customer use.
