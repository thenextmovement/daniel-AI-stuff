...
return output;` }),
    node("order-prepare", "Validate and Prepare BillingCase", "n8n-nodes-base.code", [-180, 0], { mode: "runOnceForEachItem", jsCode: "const x=$json||{};if(!x.sourceEventId||!x.shopifyOrderId||!/^#NEONT\\d+$/.test(String(x.shopifyOrderName||''))||!Array.isArray(x.lineItems)||!x.lineItems.length||!x.totals||!x.billingAddress||!x.deliveryAddress)throw new Error('invalid_shopify_order_billing_intake');return {json:{payload:x,eventId:String(x.sourceEventId),fingerprint:String(x.fingerprint)}};" }),
    node("order-post", "Create or Replay BillingCase", "n8n-nodes-base.httpRequest", [80, 0], { method: "POST", url: "https://ops.neontrip.de/api/internal/billing/cases", authentication: "genericCredentialType", genericAuthType: "httpHeaderAuth", sendHeaders: true, headerParameters: { parameters: [{ name: "X-Neontrip-Event-Id", value: "={{ $json.eventId }}" }] }, sendBody: true, specifyBody: "json", jsonBody: "={{ $json.payload }}", options: { timeout: 30000, response: { response: { fullResponse: true, responseFormat: "json" } } } }, { typeVersion: 4.3, credentials: opsCredentials, retryOnFail: true, maxTries: 3, waitBetweenTries: 3000, onError: "continueErrorOutput" }),
    node("order-mark", "Mark Shopify Order Ingested", "n8n-nodes-base.code", [320, -80], { mode: "runOnceForEachItem", jsCode: "const prepared=$('Validate and Prepare BillingCase').item.json;const state=$getWorkflowStaticData('global');state.completed=state.completed||{};state.completed[prepared.fingerprint]=new Date().toISOString();const keys=Object.keys(state.completed);for(const key of keys.slice(0,Math.max(0,keys.length-1000)))delete state.completed[key];const result={ok:true,fingerprint:prepared.fingerprint,response:$json.body??$json};return { json: result };" }),
  ...
  connections: {
    ...
    "Normalize Unseen Shopify Orders": { main: [[{ node: "Validate and Prepare BillingCase", type: "main", index: 0 }]] },
    "Validate and Prepare BillingCase": { main: [[{ node: "Create or Replay BillingCase", type: "main", index: 0 }]] },
