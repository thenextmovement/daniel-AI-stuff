...
const workflow = {
  ...
  nodes: [
    ...
    node("config", "Billing Worker Config", "n8n-nodes-base.code", [-900, 0], { mode: "runOnceForAllItems", jsCode: "const base=String($env.NEONTRIP_OPS_BASE_URL||'').replace(/\\/+$/,''); if(!/^https:\\/\\//.test(base)) throw new Error('NEONTRIP_OPS_BASE_URL missing'); return [{json:{opsBaseUrl:base,worker:'n8n-easybill-document-v2'}}];" }),
    ...
    node("done-document", "Easybill Finalize Document", "n8n-nodes-base.httpRequest", [2060, 20], { method: "PUT", url: "={{ 'https://api.easybill.de/rest/v1/documents/' + encodeURIComponent(($json.body??$json).id) + '/done' }}", authentication: "genericCredentialType", genericAuthType: "httpHeaderAuth", options: { timeout: 30000, response: { response: { fullResponse: true, responseFormat: "json" } } } }, { credentials: easybillCredentials, onError: "continueErrorOutput" }),
    node("success", "Prepare Successful Completion", "n8n-nodes-base.code", [2300, -120], { jsCode: "const ctx=$('Prepare Easybill Command').first().json; const doc=$json.easybillDocument||$json.body||$json; if(!doc.id) throw new Error('easybill_document_id_missing'); return [{json:{jobId:ctx.job.id,leaseToken:ctx.job.lease_token,success:true,result:{worker:'n8n-easybill-document-v2',easybillDocumentId:String(doc.id),documentNumber:String(doc.number||ctx.documentNumber),sent:false}}}];" }, { onError: "continueErrorOutput" }),
    node("complete", "Complete Billing Job", "n8n-nodes-base.httpRequest", [2540, -120], { method: "POST", url: "={{ $('Billing Worker Config').first().json.opsBaseUrl + '/api/internal/billing/jobs/' + encodeURIComponent($json.jobId) + '/complete' }}", authentication: "genericCredentialType", genericAuthType: "httpHeaderAuth", sendBody: true, specifyBody: "json", jsonBody: "={{ {leaseToken:$json.leaseToken,success:$json.success,result:$json.result,error:$json.error} }}", options: { timeout: 30000, response: { response: { fullResponse: true, responseFormat: "json" } } } }, { credentials: opsCredentials, onError: "continueErrorOutput" }),
  ...
  connections: {
    ...
    "Reuse Existing Document": { main: [[{ node: "Prepare Successful Completion", type: "main", index: 0 }]] },
    ...
    "Easybill Finalize Document": { main: [[{ node: "Prepare Successful Completion", type: "main", index: 0 }], [{ node: "Prepare Failed Completion", type: "main", index: 0 }]] },
