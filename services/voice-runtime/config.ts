...
export function loadRuntimeConfig() {
  ...
  return {
    ...
    aiHandoffEnabled: optional("VOICE_PHONE_AI_HANDOFF_ENABLED")==="true",
    ...
    mobilePhoneNumbers: optional("VOICE_PHONE_MOBILE_NUMBERS").split(",").map(x=>x.trim()).filter(x=>/^[+][1-9][0-9]{6,14}$/.test(x)),
