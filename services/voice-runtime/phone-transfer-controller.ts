...
import {browserCallingReady,mobileCallingReady,phoneRoom} from "./phone-calls.js";
import {PhoneTransferFlow,TwilioTransferProvider,phoneTransferTwiml,type PhoneTransfer,type TransferStore,type TransferEvent,type TransferProvider} from "./phone-transfers.js";
...
export class RuntimePhoneTransfers{
 ...
 constructor(private readonly config:RuntimeConfig,private readonly ops:TransferOps,private readonly closeCall:(call:PhoneCallRecord)=>Promise<void>,provider?:TransferProvider,private readonly startMobile?:(id:string)=>Promise<void>){
 ...
 async resume(id:string){await this.flow.resume(id);await this.dispatchMobile(id);}
 private async dispatchMobile(id:string){
  const {transfer:t}=await this.store.get(id);
  if(t.to_transport==="mobile"&&t.mobile_leg_id&&!t.ended_at&&!t.cancel_requested&&t.state==="dialing"&&t.dial_claimed&&t.customer_held)
   await this.startMobile?.(t.mobile_leg_id);
 }
 ...
 async control(input:Record<string,unknown>){
  ...
  if(input.action==="begin"){
   if(!browserCallingReady(this.config)&&!(mobileCallingReady(this.config)&&this.config.mobileTransfersEnabled))throw Error("phone_calling_not_configured");
   const {transfer:t}=await this.ops.transferAction<{transfer:PhoneTransfer}>({
    action:"begin",allowMobile:mobileCallingReady(this.config)&&this.config.mobileTransfersEnabled,allowBrowser:browserCallingReady(this.config),callId:uuid(input.callId),deviceId,targetStaffId:uuid(input.targetStaffId),requestKey:uuid(input.requestKey),
   });
   this.kick(t.id,this.flow.prepare(t.id).then(()=>this.dispatchMobile(t.id)));return {transferId:t.id};
  }
 ...
 async client(params:URLSearchParams){
  ...
  if(t.to_transport==="mobile"||t.id!==id||t.to_device_id!==deviceId||t.to_call_sid!==sid||t.ended_at||t.cancel_requested||call.ended_at||!t.customer_held||!call.conference_sid)
   throw Error("transfer_invitation_not_current");
  return phoneTransferTwiml(this.config,t,call);
 }
 ...
 async reconcile(){
  ...
  for(const row of transfers){
   try{
    ...
    }else if(["preparing","dialing"].includes(t.state)&&(Date.parse(t.expires_at)<Date.now()||!(t.to_transport==="mobile"?this.config.mobileTransfersEnabled&&this.config.mobileCallsEnabled:this.config.browserCallsEnabled))){
