...
test("shipping automation scope allows ingestion but not manual incident decisions", () => {
  assert.equal(isShippingAutomationAction("upsert_shipment"), true);
  assert.equal(isShippingAutomationAction("record_event"), true);
  assert.equal(isShippingAutomationAction("evaluate_shipment"), true);
  assert.equal(isShippingAutomationAction("create_task"), false);
  assert.equal(isShippingAutomationAction("resolve"), false);
});
