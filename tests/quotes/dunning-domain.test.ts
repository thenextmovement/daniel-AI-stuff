...
function order(overrides: Partial<BuildInput["orders"][number]> = {}): BuildInput["orders"][number] {
  return {
    ...
    ship_address: { company: "Muster GmbH", first_name: "Max", last_name: "Muster" },
    bill_address: { company: "Muster GmbH", first_name: "Max", last_name: "Muster" },
    phone: "+49 211 123456",
    ...overrides,
  };
}

function candidate(overrides: Partial<BuildInput["candidates"][number]> = {}): BuildInput["candidates"][number] {
  return {
    shopify_order_id: "1234567890",
...
test("order numbers and current stage labels are normalized deterministically", () => {
  ...
  assert.equal(dunningStageLabel(4, "legacy"), "Letzte Mahnung vor Inkasso (Altbestand)");
});

test("business-day schedules match the six-stage T099 cadence", () => {
  assert.equal(nextDunningSchedule({ nextStage: 1, orderCreatedAt: "2026-08-21T10:00:00Z", easybillCreatedAt: null, dueDate: null, previousStageSentAt: null }), "2026-08-26T10:00:00.000Z");
  assert.equal(nextDunningSchedule({ nextStage: 2, orderCreatedAt: null, easybillCreatedAt: null, dueDate: null, previousStageSentAt: "2026-08-21T10:00:00Z" }), "2026-08-27T10:00:00.000Z");
  assert.equal(nextDunningSchedule({ nextStage: 7, orderCreatedAt: null, easybillCreatedAt: null, dueDate: null, previousStageSentAt: null }), null);
});
...
test("a customer reply after the last verified stage blocks all automatic continuation", () => {
  const locks: BuildInput["locks"] = [{
    request_id: "e936881a-fe32-4d94-aa1d-eaffcf4a75be:T099:PAYMENT_COLLECTION:1234567890:S1",
  ...
  const audits: BuildInput["audits"] = [{
    ...
    error_message: null,
    metadata: {
      lock_key: "e936881a-fe32-4d94-aa1d-eaffcf4a75be:T099:PAYMENT_COLLECTION:1234567890:S1",
      stage: 1,
  ...
  assert.ok(entry?.blockers.includes("Neue Kundenantwort muss zuerst geprüft werden"));
});

test("legacy final-warning cases such as NEONT2993 go to court review, never back to stage one", () => {
  const legacyOrder = order({ shopify_order_id: "2993000", name: "#NEONT2993" });
  ...
  const [entry] = buildDunningCases(input({ orders: [legacyOrder], candidates: [], statuses }));
...
test("paid, stopped and conflicting cases fail closed", () => {
  const [paid] = buildDunningCases(input({ orders: [order({ financial_status: "paid", total_outstanding: 0 })] }));
  assert.equal(paid?.state, "closed");
  assert.equal(paid?.sendEligible, false);

  const [stopped] = buildDunningCases(input({ orders: [order({ tags: "VIP, Keine Zahlungserinnerung n8n" })] }));
  assert.equal(stopped?.state, "paused");
  assert.ok(stopped?.blockers.includes("Shopify-Sperrtag ist gesetzt"));

  const statuses: BuildInput["statuses"] = [{
    shopify_order_number: "#NEONT5000", mahnstufe: 3, last_sent_at: "2026-08-20T08:00:00.000Z",
    next_due_at: null, paused: false, note: null, updated_by: "legacy", updated_at: "2026-08-20T08:00:00.000Z",
  }];
  ...
  assert.ok(conflict?.blockers.includes("Alt- und Neuverlauf haben unterschiedliche Mahnstufen"));
});
