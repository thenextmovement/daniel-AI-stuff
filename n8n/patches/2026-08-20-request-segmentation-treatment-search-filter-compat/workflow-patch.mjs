...
import {
  ...
  WORKFLOW_ID
} from "./treatment-focus-source.mjs";

const PATCH_DIR = path.dirname(fileURLToPath(import.meta.url));
const PRESTATE_DIR = path.resolve(
  PATCH_DIR,
  "../../backups/2026-08-20-request-segmentation-treatment-search-filter-compat"
);
...
function assertPreparedPrestate(draft, active, manifest, rawDraft, rawActive) {
  ...
  if (manifest.draft_version_id !== "22463d9f-e467-45a7-b16e-5892cbbdae31"
      ...
      || draft.versionCounter !== 130
      || manifest.version_counter !== 130) {
...
export function createPatchBundle() {
  ...
  const intent =
    "Repair Treatment-focus web-search compatibility by omitting the unsupported provider-side filters while preserving the exact site query and strict returned-source domain validation.";

  return {
    artifact_version: "neontrip_n8n_treatment_search_filter_compat_v1",
    ...
    reverse: {
      id: WORKFLOW_ID,
      intent: "Restore the exact current Counter-130 v3 graph and remove only the three treatment-evaluation nodes.",
    ...
    validate_only_reverse: {
      id: WORKFLOW_ID,
      intent: "Validate the exact treatment-evaluation reverse without writing.",
      operations: reverseOperations,
      validateOnly: true,
      continueOnError: false
    },
    validate_only_roundtrip: {
      id: WORKFLOW_ID,
      intent: "Validate the treatment-evaluation patch and exact reverse without writing.",
    ...
    safety: {
      ...
      evaluation_source: "gold_re_evaluation_phase7_treatment",
