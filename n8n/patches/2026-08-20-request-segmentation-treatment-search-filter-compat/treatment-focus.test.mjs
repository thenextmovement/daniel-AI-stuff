...
test("pinned v3 graph round-trips and only the evaluation lane is patched", () => {
  ...
  assert.equal(draft.versionId, "22463d9f-e467-45a7-b16e-5892cbbdae31");
  assert.equal(draft.versionCounter, 130);
...
test("provider results outside the exact domain still fail closed without API filters", () => {
  ...
  assert.throws(
    ...
    (error) => error && error.name === "phase7_research_domain_scope_invalid"
  );
});
