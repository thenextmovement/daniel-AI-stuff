...
const PRESTATE_DIR = path.resolve(
  PATCH_DIR,
  "../../backups/2026-08-21-request-segmentation-treatment-shadow-always-on"
);
...
function assertPreparedPrestate(draft, active, manifest, rawDraft, rawActive) {
  ...
  if (manifest.draft_version_id !== "d42befa7-f6fc-4201-8516-c71c01cf5e17"
      ...
      || draft.versionCounter !== 136
      || manifest.version_counter !== 136) {
...
export function createPatchBundle() {
  ...
  const intent =
    "Run the simplified treatment classifier permanently in shadow: research exact business domains, classify freemail from request use, and persist standard versus special without customer actions.";

  return {
    artifact_version: "neontrip_n8n_treatment_shadow_always_on_v1",
    ...
    reverse: {
      id: WORKFLOW_ID,
      intent: "Restore the exact current Counter-136 v3 graph and remove only the three treatment-shadow nodes.",
    ...
    validate_only_reverse: {
      id: WORKFLOW_ID,
      intent: "Validate the exact treatment-shadow reverse without writing.",
      operations: reverseOperations,
      validateOnly: true,
      continueOnError: false
    },
    validate_only_roundtrip: {
      id: WORKFLOW_ID,
      intent: "Validate the treatment-shadow patch and exact reverse without writing.",
    ...
    safety: {
      ...
      general_ingress_processing_paused: false,
      claim_limit: 1,
      exact_versioned_general_ingress_only: true,
      policy_mode: "shadow",
      master_projection_authorized_by_job: true,
      master_projection_blocked_by_shadow_mode: true,
      customer_actions_blocked: true
    }
  };
}
...
export function createArtifactFiles() {
  ...
  if (JSON.stringify(reversed) !== JSON.stringify(draft)) {
    throw new Error("Generated treatment-evaluation reverse does not restore the full workflow.");
  }
