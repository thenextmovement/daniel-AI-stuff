...
test("current restored v3 prestate is pinned and forward/reverse is exact", () => {
  const { draft, active, manifest } = loadPreparedPrestate();
  assert.equal(draft.versionId, "1e0d0ac1-4035-4ad2-8dbb-a191a4bd5e88");
  assert.equal(draft.activeVersionId, draft.versionId);
  assert.equal(draft.versionCounter, 120);
