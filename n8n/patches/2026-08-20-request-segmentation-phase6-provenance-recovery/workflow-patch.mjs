...
import {
  ...
  WORKFLOW_ID
} from "./provenance-recovery-source.mjs";

const PATCH_DIR = path.dirname(fileURLToPath(import.meta.url));
const PRESTATE_DIR = path.resolve(
  PATCH_DIR,
  "../../backups/2026-08-20-request-segmentation-phase6-provenance-recovery"
);
...
function assertPreparedPrestate(draft, active, manifest, rawDraft, rawActive) {
  ...
  if (manifest.draft_version_id !== "1e0d0ac1-4035-4ad2-8dbb-a191a4bd5e88"
      ...
      || draft.versionCounter !== 120
      || manifest.version_counter !== 120) {
...
export function createPatchBundle() {
  ...
  const intent =
    "Repair the isolated NEONTRIP Phase-6 provenance boundary while preserving the v5 evaluation contract.";

  return {
    artifact_version: "neontrip_n8n_phase6_provenance_recovery_v1",
    ...
    reverse: {
      id: WORKFLOW_ID,
      intent: "Restore the exact current v3 graph and remove only the three Phase-6 nodes.",
    ...
    validate_only_reverse: {
      id: WORKFLOW_ID,
      intent: "Validate the exact Phase-6 reverse without writing.",
      operations: reverseOperations,
      validateOnly: true,
      continueOnError: false
    },
    validate_only_roundtrip: {
      id: WORKFLOW_ID,
      intent: "Validate the Phase-6 patch and exact reverse without writing.",
