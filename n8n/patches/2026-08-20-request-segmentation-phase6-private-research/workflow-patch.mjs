...
const PRESTATE_DIR = path.resolve(
  PATCH_DIR,
  "../../backups/2026-08-20-request-segmentation-phase6-private-research"
);
...
function assertPreparedPrestate(draft, active, manifest, rawDraft, rawActive) {
  ...
  if (manifest.draft_version_id !== "5dc73e80-3249-4985-a51b-001c5cb69223"
      ...
      || draft.versionCounter !== 118
      || manifest.version_counter !== 118) {
...
export function createPatchBundle() {
  ...
  const intent =
    "Prepare the isolated NEONTRIP Phase-6 evaluation lane with privacy-bounded two-stage research and classification.";

  return {
    artifact_version: "neontrip_n8n_phase6_private_research_v1",
    ...
    reverse: {
      id: WORKFLOW_ID,
      intent: "Restore the exact paused v3 graph and remove only the three Phase-6 nodes.",
