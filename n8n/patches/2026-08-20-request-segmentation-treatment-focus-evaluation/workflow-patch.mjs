...
const PRESTATE_DIR = path.resolve(
  PATCH_DIR,
  "../../backups/2026-08-20-request-segmentation-treatment-focus-evaluation"
);
...
function assertPreparedPrestate(draft, active, manifest, rawDraft, rawActive) {
  ...
  if (manifest.draft_version_id !== "8e783830-6a38-4dae-aa32-2daf49d1387a"
      ...
      || draft.versionCounter !== 128
      || manifest.version_counter !== 128) {
...
export function createPatchBundle() {
  ...
  const intent =
    "Evaluate the treatment-focused v6 contract only: exact-domain research, request evidence for standard cases, and external evidence for special handling.";

  return {
    artifact_version: "neontrip_n8n_treatment_focus_evaluation_v1",
    ...
    reverse: {
      id: WORKFLOW_ID,
      intent: "Restore the exact current Counter-128 v3 graph and remove only the three treatment-evaluation nodes.",
