...
function payload({ freemail = false, description, extraInput = {}, extraContract = {} } = {}) {
...
function runBuild(inputPayload) {
...
test("pinned v3 graph round-trips and only the evaluation lane is patched", () => {
  const { draft, active } = loadPreparedPrestate();
  assert.equal(draft.versionId, "8e783830-6a38-4dae-aa32-2daf49d1387a");
  assert.equal(draft.versionCounter, 128);
...
test("business-domain research is exact-domain-only and contains no request text or ids", () => {
  ...
  assert.deepEqual(body.tools[0].filters, { allowed_domains: ["example.com"] });
...
test("freemail private-use wording accepts NT-8 as standard request evidence", () => {
  ...
  assert.equal(result.rpcBody.p_treatment_contract, TREATMENT_CONTRACT);
...
test("ambiguous freemail remains needs_review instead of guessing private", () => {
  const { prepared } = prepareFreemail();
  const result = runValidator(classifierResponse(model({ decision: "needs_review" })), prepared);
...
test("missing provider citations do not block a clearly standard request", () => {
  ...
  assert.equal(prepared.researchEvidence.abstention_code, "phase7_research_provenance_missing");
...
test("special public, multisite, enterprise, or large results require bound web evidence", () => {
  ...
  assert.equal(blocked.rpcBody.p_segment, "NT-10");
...
test("Stage-2 stays tool-free and the record RPC has exactly 20 named arguments", () => {
  const { prepared } = prepareBusiness();
  ...
  standard.firmographic.email_domain = "example.com";
  const result = runValidator(classifierResponse(standard), prepared);
  assert.equal(Object.keys(result.rpcBody).length, 20);
